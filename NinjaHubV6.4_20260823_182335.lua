...
local States = {
	...
	BigSpin = {Enabled = false, Speed = 200},
...
-- ============================================
-- 功能列表 HUD (FeatureHUD) - 独立组件版
-- ============================================
do
	--================ 功能列表 HUD (独立组件式) ================
	-- 每个功能都是独立的圆角矩形组件, 带彩色渐变描边
	-- 开启: 从左侧丝滑向右平移进入右上角并依次向下堆叠
	-- 关闭: 丝滑向右滑出离开
	-- 互不依赖, 绝不重叠
	local F = {}
	F.Comps = {}
	...
	local function textW(str)
		...
		if ok and typeof(b) == "Vector2" and b.X and b.X > 0 then return b.X + 4 end
		return #str * TEXTSIZE + 4
	end
	...
	local function slotY(i) return TOP + (i - 1) * (ROW_H + GAP) end

	--=== 创建单个独立组件 ===
	local function makeComp(key)
		local s = States.FeatureHUD
		local w = math.max(70, textW(nameOf[key] or key) + PAD_X * 2)
		local c = Instance.new("Frame")
		c.Name = "NH_FComp_" .. key
		c.AnchorPoint = Vector2.new(1, 0)
		c.Size = UDim2.new(0, w, 0, ROW_H)
		c.BackgroundColor3 = Color3.fromRGB(16, 12, 34)
		c.BackgroundTransparency = 0.65
		c.BorderSizePixel = 0
		c.Active = false
		c.Selectable = false
		c.ZIndex = 9800
		c.Parent = ScreenGui
		local corner = Instance.new("UICorner"); corner.CornerRadius = UDim.new(0, 9); corner.Parent = c
		-- 彩色渐变描边(轮廓)
		local stroke = Instance.new("UIStroke")
		stroke.Thickness = 2
		...
		name.Size = UDim2.new(1, -PAD_X * 2, 1, 0)
		name.Position = UDim2.new(0, PAD_X - 8, 0, 0)
		...
		name.ZIndex = 9807
		name.Parent = c
		c.Resources = {stroke = sg, strokeObj = stroke, name = name}
		F.Comps[key] = c
		return c
	end

	--=== 开启: 从左向右滑入 ===
	local function spawn(key, i)
		local c = makeComp(key)
		local y = slotY(i)
		-- 从左侧较远位置开始, 向右平移进入最终槽位
		c.Position = UDim2.new(1, -(RIGHT + 150), 0, y)
		local res = c.Resources
		res.name.TextTransparency = 1
		res.strokeObj.Transparency = 1
		c.BackgroundTransparency = 1
		tween(c, {Position = UDim2.new(1, -RIGHT, 0, y)}, TweenSlide)
		tween(c, {BackgroundTransparency = 0.65}, TweenSmooth)
		tween(res.name, {TextTransparency = 0}, TweenSmooth)
		tween(res.strokeObj, {Transparency = 0}, TweenSmooth)
		return c
	end
	...
	local function despawn(key)
		...
		pcall(function()
			tween(c, {Position = UDim2.new(1, 220, 0, y)}, TweenSlide)
			tween(c, {BackgroundTransparency = 1}, TweenFast)
			tween(res.name, {TextTransparency = 1}, TweenFast)
			tween(res.strokeObj, {Transparency = 1}, TweenFast)
	...
	local function relayout()
		...
		F.Order = list
		local gone = {}
		for key in pairs(F.Comps) do
			if not isIn(list, key) then table.insert(gone, key) end
		end
		for _, key in ipairs(gone) do despawn(key) end
		for i, key in ipairs(list) do
			...
			if not c then
				...
				local y = slotY(i)
				if math.abs(c.Position.Y.Offset - y) > 1 then
					tween(c, {Position = UDim2.new(1, -RIGHT, 0, y)}, TweenSlide)
				else
					c.Position = UDim2.new(1, -RIGHT, 0, y)
				end
			end
		end
	end
	...
	local function applyColors(now)
		...
		for i, key in ipairs(list) do
			...
			local col2 = gradColor(t + 0.09)
			local res = c.Resources
			res.stroke.Color = ColorSequence.new(col, col2)
	...
	F.reapply = function()
		if not States.FeatureHUD.Enabled then
			for key, c in pairs(F.Comps) do pcall(function() c:Destroy() end) end
			F.Comps = {}
			F.Order = {}
			return
		end

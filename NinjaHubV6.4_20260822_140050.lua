...
local States = {
	...
	ClientScript = {Enabled = false, Recording = false, Playing = false, Steps = {}, RepeatCount = 1, SavedScripts = {}},
	-- V6.3 新功能
...
local ClickScriptThread, ClientScriptThread
...
local function buildClientScriptPanel()
	...
	-- 左侧保存列表
	local saveListFrame = Instance.new("ScrollingFrame")
	saveListFrame.Size = UDim2.new(0, 90, 0, 200)
	saveListFrame.Position = UDim2.new(0, 8, 0, 34)
	saveListFrame.BackgroundTransparency = 1
	saveListFrame.ScrollBarThickness = 2
	saveListFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
	saveListFrame.Parent = panel
	...
	listFrame.Position = UDim2.new(0, 104, 0, 34)
	listFrame.BackgroundTransparency = 1
	listFrame.ScrollBarThickness = 2
	listFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
	listFrame.Parent = panel
	...
	local stepsTitle = Instance.new("TextLabel")
	stepsTitle.Size = UDim2.new(1, 0, 0, 18)
	stepsTitle.Position = UDim2.new(0, 104, 0, 34)
	stepsTitle.BackgroundTransparency = 1
	stepsTitle.Text = "📝 当前步骤"
	stepsTitle.TextColor3 = Color3.fromRGB(200, 200, 255)
	stepsTitle.TextSize = 10
	...
	local ctrlRow = Instance.new("Frame")
	ctrlRow.Size = UDim2.new(1, 0, 0, 26)
	ctrlRow.Position = UDim2.new(0, 0, 0, 240)
	ctrlRow.BackgroundTransparency = 1
	ctrlRow.Parent = panel

	local recordBtn = createButton(ctrlRow, "Record", UDim2.new(0, 56, 0, 24), UDim2.new(0, 0, 0, 0), Color3.fromRGB(255, 100, 50), "⏺ 记录")
	recordBtn.TextSize = 10
	local saveBtn = createButton(ctrlRow, "SaveScript", UDim2.new(0, 56, 0, 24), UDim2.new(0, 58, 0, 0), Color3.fromRGB(0, 100, 180), "💾 保存")
	saveBtn.TextSize = 10
	local playBtn = createButton(ctrlRow, "Play", UDim2.new(0, 56, 0, 24), UDim2.new(0, 116, 0, 0), Color3.fromRGB(0, 150, 80), "▶ 执行")
	playBtn.TextSize = 10
	local stopBtn = createButton(ctrlRow, "Stop", UDim2.new(0, 56, 0, 24), UDim2.new(0, 174, 0, 0), Color3.fromRGB(200, 60, 60), "⏹ 停止")
	stopBtn.TextSize = 10
	local delBtn = createButton(ctrlRow, "Delete", UDim2.new(0, 56, 0, 24), UDim2.new(0, 232, 0, 0), Color3.fromRGB(150, 50, 50), "🗑")
	delBtn.TextSize = 10

	-- 循环次数
	local repeatRow = Instance.new("Frame")
	repeatRow.Size = UDim2.new(1, 0, 0, 24)
	repeatRow.Position = UDim2.new(0, 0, 0, 270)
	repeatRow.BackgroundTransparency = 1
	repeatRow.Parent = panel

	local repeatLabel = Instance.new("TextLabel")
	repeatLabel.Size = UDim2.new(0, 50, 0, 24)
	repeatLabel.Position = UDim2.new(0, 8, 0, 0)
	repeatLabel.BackgroundTransparency = 1
	repeatLabel.Text = "循环:"
	repeatLabel.TextColor3 = Color3.fromRGB(200, 200, 255)
	...
	local repeatBox = Instance.new("TextBox")
	repeatBox.Size = UDim2.new(0, 40, 0, 22)
	repeatBox.Position = UDim2.new(0, 52, 0, 1)
	repeatBox.BackgroundColor3 = C.BtnDark
	repeatBox.Text = tostring(States.ClientScript.RepeatCount)
	repeatBox.TextColor3 = Color3.fromRGB(255, 255, 255)
	repeatBox.TextSize = 10
	repeatBox.Font = Enum.Font.Gotham
	repeatBox.Parent = repeatRow
	...
	local repeatMinus = createButton(repeatRow, "RepMinus2", UDim2.new(0, 20, 0, 22), UDim2.new(0, 96, 0, 1), C.Btn, "-")
	local repeatPlus = createButton(repeatRow, "RepPlus2", UDim2.new(0, 20, 0, 22), UDim2.new(0, 118, 0, 1), C.Btn, "+")

	repeatMinus.MouseButton1Click:Connect(function()
		States.ClientScript.RepeatCount = math.max(1, States.ClientScript.RepeatCount - 1)
		repeatBox.Text = tostring(States.ClientScript.RepeatCount)
	end)
	repeatPlus.MouseButton1Click:Connect(function()
		States.ClientScript.RepeatCount = math.min(999, States.ClientScript.RepeatCount + 1)
		repeatBox.Text = tostring(States.ClientScript.RepeatCount)
	end)
	repeatBox.FocusLost:Connect(function()
		local n = tonumber(repeatBox.Text)
		if n and n >= 1 then
			States.ClientScript.RepeatCount = math.floor(n)
			repeatBox.Text = tostring(States.ClientScript.RepeatCount)
		end
	end)
	...
	statusLabel.TextXAlignment = Enum.TextXAlignment.Left
	statusLabel.Parent = panel
	Gui.ClientScriptStatus = statusLabel

	-- 记录条
	RecordBar = Instance.new("TextLabel")
	RecordBar.Size = UDim2.new(0, 260, 0, 26)
	RecordBar.Position = UDim2.new(0.5, -130, 0, 8)
	RecordBar.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	RecordBar.BackgroundTransparency = 0.4
	RecordBar.Text = ""
	RecordBar.TextColor3 = Color3.fromRGB(255, 220, 120)
	RecordBar.TextSize = 10
	RecordBar.Font = Enum.Font.GothamBold
	RecordBar.Visible = false
	RecordBar.ZIndex = 9600
	RecordBar.Parent = ScreenGui
	local rbc2 = Instance.new("UICorner"); rbc2.CornerRadius = UDim.new(0, 8); rbc2.Parent = RecordBar
	createGrayStroke(RecordBar, 1.5)
	...
	function renderSaveList()
		...
		local saves = States.ClientScript.SavedScripts
		if #saves == 0 then
			local empty = Instance.new("TextLabel")
		...
		for i, save in ipairs(saves) do
			...
			row.MouseButton1Click:Connect(function()
				States.ClientScript.Steps = table.clone(save.Steps)
				renderClientScriptSteps()
				statusLabel.Text = "📂 已加载: " .. (save.Name or "脚本" .. i)
			end)
			local delBtn = createButton(row, "DelSave"..i, UDim2.new(0, 16, 0, 16), UDim2.new(1, -18, 0.5, -8), Color3.fromRGB(200, 60, 60), "×")
			delBtn.TextSize = 8
			delBtn.MouseButton1Click:Connect(function()
				table.remove(saves, i)
				renderSaveList()
	...
	-- 渲染步骤
	function renderClientScriptSteps()
		for _, c in ipairs(listFrame:GetChildren()) do
			if c:IsA("Frame") or c:IsA("TextButton") then c:Destroy() end
		end
		local steps = States.ClientScript.Steps
		if #steps == 0 then
			local empty = Instance.new("TextLabel")
			empty.Size = UDim2.new(1, 0, 0, 20)
			empty.BackgroundTransparency = 1
		...
		for i, step in ipairs(steps) do
			...
			typeLabel.Position = UDim2.new(0, 24, 0.5, -10)
			typeLabel.BackgroundColor3 = C.BtnDark
			typeLabel.BackgroundTransparency = 0.2
			typeLabel.Text = step.Type or "移动"
			typeLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
			typeLabel.TextSize = 9
			typeLabel.Font = Enum.Font.Gotham
			typeLabel.Parent = row
			local tlc = Instance.new("UICorner"); tlc.CornerRadius = UDim.new(0, 5); tlc.Parent = typeLabel

			local detailLabel = Instance.new("TextLabel")
			detailLabel.Size = UDim2.new(0, 100, 0, 20)
			detailLabel.Position = UDim2.new(0, 88, 0.5, -10)
			detailLabel.BackgroundTransparency = 1
			detailLabel.Text = step.Detail or ""
			detailLabel.TextColor3 = Color3.fromRGB(200, 200, 255)
			...
			local delBtn = createButton(row, "DelStep"..i, UDim2.new(0, 18, 0, 18), UDim2.new(1, -20, 0.5, -9), Color3.fromRGB(200, 60, 60), "×")
			delBtn.TextSize = 8
			delBtn.MouseButton1Click:Connect(function()
				table.remove(States.ClientScript.Steps, i)
				renderClientScriptSteps()
			end)
		end
	end
	...
	-- 记录逻辑
	local recordStartPos = nil
	local recordStartTime = 0
	local lastRecordStep = 0
	local lastRecordPos = Vector3.zero
	local lastInteractTime = 0
	local lastToolName = nil

	local function startRecording()
		if States.ClientScript.Recording then return end
		States.ClientScript.Recording = true
		States.ClientScript.Steps = {}
		recordStartPos = hrp and hrp.Position or Vector3.zero
		...
		statusLabel.Text = "⏺ 记录中..."
		RecordBar.Visible = true

		local recordConn = RunService.Heartbeat:Connect(function()
			if not States.ClientScript.Recording then
				recordConn:Disconnect()
				return
			end
			if not hrp then return end
			local curPos = hrp.Position
			local dist = (curPos - recordStartPos).Magnitude
			local moved = (curPos - lastRecordPos).Magnitude
			if moved >= 0.5 then
				local stepIndex = #States.ClientScript.Steps + 1
				table.insert(States.ClientScript.Steps, {
					Type = "移动",
					Detail = string.format("向前 %.1fm", dist),
					Position = curPos,
					Time = tick() - recordStartTime,
				})
				lastRecordPos = curPos
				lastRecordStep = stepIndex
				RecordBar.Text = string.format("⏺ 移动 %.1fm | 坐标: %.0f, %.0f, %.0f", dist, curPos.X, curPos.Y, curPos.Z)
				renderClientScriptSteps()
			else
				RecordBar.Text = string.format("⏺ 坐标: %.0f, %.0f, %.0f", curPos.X, curPos.Y, curPos.Z)
			end
		end)

		local interactConn = UserInputService.InputBegan:Connect(function(input, gpe)
			if not States.ClientScript.Recording then
			...
			if gpe then return end
			local now = tick()
			if now - lastInteractTime < 0.5 then return end
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				lastInteractTime = now
				local pos = input.Position
				-- V6.3: 通过相机射线检测实际点击的3D对象，记录真实互动目标
				local targetName = "空间"
				local targetPos = nil
				pcall(function()
					if camera and camera.Parent then
						...
						local filter = {}
						if character then filter[character] = true end
						...
						if hit and hit.Instance then
							...
							if inst:IsA("Tool") or (inst.Parent and inst.Parent:IsA("Tool")) then
								targetName = "道具/" .. inst.Name
							elseif inst.Parent and inst.Parent.Parent and inst.Parent.Parent:FindFirstChildOfClass("Humanoid") then
								targetName = "角色/" .. inst.Name
							end
						end
					end
				end)
				...
				RecordBar.Text = string.format("⏺ 互动: %s | 步骤 %d", targetName, #States.ClientScript.Steps)
				renderClientScriptSteps()
			end
		end)

		local toolConn = player.Character and player.Character.ChildAdded or nil
		if toolConn then
			toolConn:Connect(function(child)
				if not States.ClientScript.Recording then return end
				if child:IsA("Tool") and child.Name ~= lastToolName then
					lastToolName = child.Name
					table.insert(States.ClientScript.Steps, {
						Type = "切换道具",
						Detail = child.Name,
						Position = hrp and hrp.Position or Vector3.zero,
					...
					RecordBar.Text = string.format("⏺ 道具: %s", child.Name)
					renderClientScriptSteps()
				end
			end)
		end

		States.ClientScript._recordConn = recordConn
		States.ClientScript._interactConn = interactConn
	end

	local function stopRecording()
		if not States.ClientScript.Recording then return end
		States.ClientScript.Recording = false
		if States.ClientScript._recordConn then
			States.ClientScript._recordConn:Disconnect()
			States.ClientScript._recordConn = nil
		end
		if States.ClientScript._interactConn then
			States.ClientScript._interactConn:Disconnect()
			States.ClientScript._interactConn = nil
		end
		recordBtn.Text = "⏺ 记录"
		recordBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 50)
		statusLabel.Text = "⏹ 记录已停止"
		RecordBar.Visible = false
	end

	local function playClientScript()
		if States.ClientScript.Playing then return end
		if #States.ClientScript.Steps == 0 then
			statusLabel.Text = "⚠ 请先记录脚本"
			return
		end
		States.ClientScript.Playing = true
		statusLabel.Text = "▶ 执行中..."
		local repeatCount = States.ClientScript.RepeatCount or 1
		local rep = 0
		while rep < repeatCount or repeatCount == 0 do
			if not States.ClientScript.Playing then break end
			local startPos = hrp and hrp.Position or Vector3.zero
			for i, step in ipairs(States.ClientScript.Steps) do
				if not States.ClientScript.Playing then break end
				statusLabel.Text = string.format("▶ %d/%d (循环%d)", i, #States.ClientScript.Steps, rep + 1)
				if step.Type == "移动" and step.Position then
					...
					if hrp then
						...
						if dist > 0.1 then
							local moveSpeed = 50
							local steps = math.ceil(dist / moveSpeed * 60)
							for s = 1, steps do
								if not States.ClientScript.Playing then break end
								local t = s / steps
								local curPos = startPos:Lerp(targetPos, t)
								hrp.CFrame = CFrame.new(curPos, curPos + dir)
								task.wait(1/60)
							end
						end
					end
				elseif step.Type == "互动" and step.ScreenPos then
					local pos = step.ScreenPos
					-- V6.3: 若记录了3D目标坐标，则重新投影到当前屏幕再点击，提升准确率
					if step.TargetPos and camera and camera.Parent then
						pcall(function()
							local screenPos, onScreen = camera:WorldToScreenPoint(step.TargetPos)
							if onScreen then
					...
					task.wait(0.1)
				elseif step.Type == "切换道具" then
					for _, tool in ipairs(player.Backpack:GetChildren()) do
						if tool:IsA("Tool") and tool.Name == step.Detail then
		...
		States.ClientScript.Playing = false
		statusLabel.Text = "✅ 执行完成"
	end
	...
	local function saveScript()
		...
		local name = "客户端脚本" .. (#States.ClientScript.SavedScripts + 1)
		table.insert(States.ClientScript.SavedScripts, {
			Name = name,
			Steps = table.clone(States.ClientScript.Steps),
			Time = os.time()
		})
		renderSaveList()
		statusLabel.Text = "💾 已保存: " .. name
	...
	recordBtn.MouseButton1Click:Connect(function()
		if States.ClientScript.Recording then
			stopRecording()
		else
			startRecording()
		end
	end)
	playBtn.MouseButton1Click:Connect(playClientScript)
	stopBtn.MouseButton1Click:Connect(function()
		States.ClientScript.Playing = false
		States.ClientScript.Recording = false
		statusLabel.Text = "⏹ 已停止"
...
local function syncClickerBallVisibility()
	local show = States.AutoClicker.Enabled or States.ClickScript.Enabled or States.ClientScript.Enabled
	for _, ball in ipairs(Gui.ClickerBalls) do
...
Updaters.ClientScript = function()
	-- 客户端脚本面板显示/隐藏
	if ClientScriptPanel then
		ClientScriptPanel.Visible = States.ClientScript.Enabled
	end
	if not States.ClientScript.Enabled then
		States.ClientScript.Recording = false
		States.ClientScript.Playing = false
		if ClientScriptThread then
			ClientScriptThread = nil
		end
		if RecordBar then
			RecordBar.Visible = false
		end
	...
	while true do
		...
		for key, state in pairs(States) do
			if type(state) == "table" and state.Enabled and Updaters[key] then
				local skip = key == "AutoSave" or key == "AntiAfk" or key == "ClickerStart" or key == "DynamicIsland" or key == "MusicPlayer" or key == "ClickScript" or key == "ClientScript" or key == "RemoteInteract"

...
local function setFeatureState(key, enabled)
	...
	if UpdateFeatureHud then pcall(UpdateFeatureHud, key, enabled) end
end
...
-- V6.5: 3D框线框材质(只绘12条棱, 不染色面, LineHandleAdornment不偏移)
local WirePools = {
	Box = {Edges = {}},
}
local function getWireEdge(pool, p1, p2, color)
	for _, e in ipairs(pool.Edges) do
		if not e.Visible then
			e.Color3 = color
			placeWireEdge(e, p1, p2)
			e.Visible = true
			return e
		end
	end
	local e = Instance.new("LineHandleAdornment")
	e.Adornee = Workspace.Terrain
	e.AlwaysOnTop = true
	e.Transparency = 0
	e.Color3 = color
	e.ZIndex = 0
	pcall(function() e.Thickness = 0.03 end)
	e.Parent = Workspace
	table.insert(pool.Edges, e)
	placeWireEdge(e, p1, p2)
	e.Visible = true
	return e
end
local function placeWireEdge(e, p1, p2)
	local dir = p2 - p1
	local len = dir.Magnitude
	if len < 0.01 then e.Visible = false return end
	local mid = (p1 + p2) / 2
	-- LineHandleAdornment 沿本地方向Z轴绘制, 故 lookAt 目标方向并旋转镂成 Z 轴
	e.CFrame = CFrame.lookAt(mid, p2) * CFrame.Angles(0, 0, math.pi / 2)
	e.Length = len
	e.Width = 0.075
	e.AlwaysOnTop = true
end
local function clearWire(pool)
	for _, e in ipairs(pool.Edges) do
		e.Visible = false
	end
end
...
local function drawBox3D(pool, char, color)
	...
	local c = cframe.Position
	local hx, hy, hz = sx / 2, sy / 2, sz / 2
	local pts = {}
	for _, cn in ipairs(BoxCorners) do
		table.insert(pts, c + right * (cn[1] * hx) + up * (cn[2] * hy) + look * (cn[3] * hz))
	end
	local wp = WirePools.Box
	for _, ed in ipairs(BoxEdges) do
		getWireEdge(wp, pts[ed[1]], pts[ed[2]], color)
	end
	return true
end

-- 命中盒依旧使用实体(填充)BoxHandleAdornment —— 保持原有核心逻辑
...
local function drawHLine(pool, parent, x, y, w, color, t)
	...
	line.Size = UDim2.new(0, w, 0, t or 0.35)
...
local function drawVLine(pool, parent, x, y, h, color, t)
	local line = getFromPool(pool, parent)
	line.AnchorPoint = Vector2.new(0, 0)
	line.Size = UDim2.new(0, t or 0.35, 0, h)
	line.Position = UDim2.new(0, x, 0, y)
	line.Rotation = 0
	line.BackgroundColor3 = color
	line.Parent = parent
...
local Features = {
	...
	{Cat=6, Name="功能HUD", Key="FeatureHud"},
}

-- ============================================
-- V6.5: 大陀螺 / 上帝模式 / 功能HUD / 关闭脚本
-- ============================================

-- 1) 大陀螺: 人物持续绕自身Y轴旋转(不改变位置, 不影响视角与行走)
States.Spin = States.Spin or {Enabled = false, Value = 360}
Updaters.Spin = function()
	if States.Spin.Enabled then
		if Conns.Spin then return end
		local spd = States.Spin.Value or 360
		Conns.Spin = RunService.RenderStepped:Connect(function(dt)
			if not States.Spin.Enabled then return end
			if hrp then
				hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(spd * dt), 0)
			end
		end)
	else
		unbind("Spin")
...
-- 2) 上帝模式(灵魂出窍): 无敌+虚无穿墙, 不可触碰互动
States.SoulGod = States.SoulGod or {Enabled = false}
Updaters.SoulGod = function()
	if States.SoulGod.Enabled then
		if Conns.SoulGod then return end
		Conns.SoulGod = RunService.Heartbeat:Connect(function()
			if not States.SoulGod.Enabled then return end
			if humanoid then
			...
			if character then
				for _, part in pairs(character:GetDescendants()) do
					if part:IsA("BasePart") then
						...
						part.CanTouch = false
						pcall(function() part.Grabbable = false end)
					end
				end
			end
		end)
...
-- 3) 功能列表HUD -------------------------------------------------
States.FeatureHud = States.FeatureHud or {
	Enabled = false, Panel = true, Display = "Text", Bg = true, Bold = false,
	AnimDir = "Forward", Palette = "Rainbow",
}
local HudCFG = States.FeatureHud
local HudPanel
local HudRows
local HudGradient
local HudChips = {}
...
local function hudFeatureName(key)
	for i, f in ipairs(Features) do
		if f.Key == key then return f.Name, i end
	end
	return key, nil
end

local function hudPaletteSeq()
	local p = HudCFG.Palette or "Rainbow"
	if p == "Red" then
		return ColorSequence.new(Color3.fromRGB(255,90,80), Color3.fromRGB(150,20,20))
	elseif p == "Blue" then
		return ColorSequence.new(Color3.fromRGB(80,150,255), Color3.fromRGB(20,60,220))
	elseif p == "Green" then
		return ColorSequence.new(Color3.fromRGB(80,255,140), Color3.fromRGB(20,150,70))
	elseif p == "Pink" then
		return ColorSequence.new(Color3.fromRGB(255,120,200), Color3.fromRGB(190,40,120))
	elseif p == "Fire" then
		return ColorSequence.new(Color3.fromRGB(255,230,80), Color3.fromRGB(255,80,40))
	else
		local keys = {}
		for i = 0, 7 do
			table.insert(keys, ColorSequenceKeypoint.new(i / 7, Color3.fromHSV((i / 7) % 1, 1, 1)))
		end
		return ColorSequence.new(keys)
	end
end

local function hudApplyStyle(chip)
	local label = chip:FindFirstChild("Lbl")
	local bg = chip:FindFirstChild("Bg")
	local l1 = chip:FindFirstChild("Ln1")
	local l2 = chip:FindFirstChild("Ln2")
	local disp = HudCFG.Display or "Text"
	if label then
		label.Font = HudCFG.Bold and Enum.Font.GothamBold or Enum.Font.Gotham
	end
	if bg then bg.Visible = HudCFG.Bg end
	if l1 then l1.Visible = (disp == "Line" or disp == "Double") end
	if l2 then l2.Visible = (disp == "Double") end
end

local function hudChipEnter(inner)
	local t0 = -44
	if HudCFG.AnimDir == "Reverse" then t0 = 44 end
	inner.Position = UDim2.new(0, t0, 0, 0)
	tween(inner, {Position = UDim2.new(0, 0, 0, 0)}, TweenSmooth)
end

local function hudAddChip(key, name, ix)
	if HudChips[key] or not HudRows or not HudPanel then return end
	...
	bg.Name = "Bg"
	bg.Size = UDim2.new(1, -20, 1, -4)
	bg.Position = UDim2.new(0, 10, 0, 2)
	bg.BackgroundColor3 = Color3.fromRGB(20, 16, 50)
	bg.BackgroundTransparency = 0.55
	bg.BorderSizePixel = 0
	bg.Parent = inner
	local bc = Instance.new("UICorner"); bc.CornerRadius = UDim.new(0, 8); bc.Parent = bg
	createGrayStroke(bg, 1)

	local l1 = Instance.new("Frame")
	l1.Name = "Ln1"
	l1.Size = UDim2.new(1, -24, 0, 1)
	l1.Position = UDim2.new(0, 12, 0, 3)
	l1.BackgroundColor3 = Color3.fromHSV((ix % 14) / 14, 1, 1)
	...
	l2.Name = "Ln2"
	l2.Size = UDim2.new(1, -24, 0, 1)
	l2.Position = UDim2.new(0, 12, 1, -4)
	l2.BackgroundColor3 = Color3.fromHSV(((ix + 7) % 14) / 14, 1, 1)
	...
	HudChips[key] = chip
	HudInners[key] = inner
	inner.LayoutOrder = 99
end

local function hudRemoveChip(key)
	local chip = HudChips[key]
	local inner = HudInners[key]
	HudChips[key] = nil
	HudInners[key] = nil
	...
	if HudCFG.AnimDir == "Reverse" then t0 = 44 end
	if inner then
		tween(inner, {Position = UDim2.new(0, t0, 0, 0)}, TweenSmooth)
	end
...
local function hudRefresh()
	for k in pairs(HudInners) do HudInners[k] = nil end
	for key, c in pairs(HudChips) do
		pcall(function() c:Destroy() end)
	end
	HudChips = {}
	if not HudRows then return end
	for i, f in ipairs(Features) do
		local st = States[f.Key]
		if st and st.Enabled then hudAddChip(f.Key, f.Name, i) end
	end
end

local function hudGradLoop()
	while not HudDead do
		if HudPanel and HudPanel.Visible and HudGradient then
			local target = HudGradient.Offset.Y > 0.5 and 0 or 1
			tween(HudGradient, {Offset = Vector2.new(0, target)}, TweenInfo.new(7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut))
		end
		task.wait(7)
	end
end

local HudBtnStyle, HudBtnBg, HudBtnBold, HudBtnAnim, HudBtnPal
local function syncHudBtns()
	if HudBtnStyle then
		local names = {Text = "▶ 样式:文本", Line = "▶ 样式:线条", Double = "▶ 样式:双线"}
		HudBtnStyle.Text = names[HudCFG.Display] or HudCFG.Display
	end
	if HudBtnBg then HudBtnBg.Text = (HudCFG.Bg and "背景:开" or "背景:关") end
	if HudBtnBold then HudBtnBold.Text = (HudCFG.Bold and "加粗:开" or "加粗:关") end
	if HudBtnAnim then
		local names = {Forward = "动画:正", Reverse = "动画:反", Bi = "动画:双向"}
		HudBtnAnim.Text = names[HudCFG.AnimDir] or HudCFG.AnimDir
	end
	if HudBtnPal then
		local names = {Rainbow = "渐变:彩", Red = "渐变:红", Blue = "渐变:蓝", Green = "渐变:绿", Pink = "渐变:粉", Fire = "渐变:火"}
		HudBtnPal.Text = names[HudCFG.Palette] or HudCFG.Palette
	end
end

local function hudReflow()
	if not HudPanel then return end
	if HudCFG.Panel then
		HudPanel.Size = UDim2.new(0, 244, 0, 238)
		if HudRows then HudRows.Visible = true end
		for _, b in ipairs(HudPanel:GetChildren()) do
			if b:IsA("Frame") and b ~= HudRows and b.Name ~= "Header" then b.Visible = true end
		end
	else
		HudPanel.Size = UDim2.new(0, 244, 0, 28)
		if HudRows then HudRows.Visible = false end
		for _, b in ipairs(HudPanel:GetChildren()) do
			if b:IsA("Frame") and b ~= HudRows and b ~= HudGradient then b.Visible = false end
		end
	end
end

local function hudGrowGradient()
	if HudGradient then
		pcall(function() HudGradient.Color = hudPaletteSeq() end)
	end
end

local function buildHudUI()
	HudPanel = Instance.new("Frame")
	HudPanel.Name = "FeatureHudPanel"
	HudPanel.Size = UDim2.new(0, 244, 0, 238)
	HudPanel.AnchorPoint = Vector2.new(1, 0)
	HudPanel.Position = UDim2.new(1, -12, 0, 96)
	HudPanel.BackgroundColor3 = Color3.fromRGB(18, 14, 46)
	HudPanel.BackgroundTransparency = 0.12
	HudPanel.BorderSizePixel = 0
	HudPanel.ZIndex = 9550
	HudPanel.ClipsDescendants = true
	HudPanel.Visible = false
	HudPanel.Parent = ScreenGui

	HudGradient = Instance.new("UIGradient")
	HudGradient.Color = hudPaletteSeq()
	HudGradient.Rotation = 90
	HudGradient.Parent = HudPanel
	local pc = Instance.new("UICorner"); pc.CornerRadius = UDim.new(0, 12); pc.Parent = HudPanel
	createGrayStroke(HudPanel, 1.5)
	...
	header.AutoButtonColor = false
	header.Parent = HudPanel
	header.MouseButton1Click:Connect(function()
		HudCFG.Panel = not HudCFG.Panel
		hudReflow()
	end)

	-- 配置按钮(两行)
	HudBtnStyle = createButton(HudPanel, "HudStyle", UDim2.new(0, 70, 0, 20), UDim2.new(0, 12, 0, 30), C.BtnDark, "样式")
	HudBtnBg = createButton(HudPanel, "HudBg", UDim2.new(0, 70, 0, 20), UDim2.new(0, 86, 0, 30), C.BtnDark, "背景")
	HudBtnBold = createButton(HudPanel, "HudBold", UDim2.new(0, 70, 0, 20), UDim2.new(0, 160, 0, 30), C.BtnDark, "加粗")
	HudBtnAnim = createButton(HudPanel, "HudAnim", UDim2.new(0, 108, 0, 20), UDim2.new(0, 12, 0, 54), Color3.fromRGB(60,42,120), "动画")
	HudBtnPal = createButton(HudPanel, "HudPal", UDim2.new(0, 108, 0, 20), UDim2.new(0, 124, 0, 54), Color3.fromRGB(60,42,120), "渐变")
	for _, b in ipairs({HudBtnStyle, HudBtnBg, HudBtnBold, HudBtnAnim, HudBtnPal}) do
		b.TextSize = 10
		b.Font = Enum.Font.GothamSemibold
	end
	HudBtnStyle.MouseButton1Click:Connect(function()
		local order = {Text = "Line", Line = "Double", Double = "Text"}
		HudCFG.Display = order[HudCFG.Display] or "Text"
		syncHudBtns()
		for k, c in pairs(HudChips) do hudApplyStyle(c) end
	end)
	HudBtnBg.MouseButton1Click:Connect(function()
		HudCFG.Bg = not HudCFG.Bg
		syncHudBtns()
		for k, c in pairs(HudChips) do hudApplyStyle(c) end
	end)
	HudBtnBold.MouseButton1Click:Connect(function()
		HudCFG.Bold = not HudCFG.Bold
		syncHudBtns()
		for k, c in pairs(HudChips) do hudApplyStyle(c) end
	end)
	HudBtnAnim.MouseButton1Click:Connect(function()
		local order = {Forward = "Reverse", Reverse = "Bi", Bi = "Forward"}
		HudCFG.AnimDir = order[HudCFG.AnimDir] or "Forward"
		syncHudBtns()
	end)
	HudBtnPal.MouseButton1Click:Connect(function()
		local order = {Rainbow = "Red", Red = "Blue", Blue = "Green", Green = "Pink", Pink = "Fire", Fire = "Rainbow"}
		HudCFG.Palette = order[HudCFG.Palette] or "Rainbow"
		syncHudBtns()
		hudGrowGradient()
	end)

	HudRows = Instance.new("ScrollingFrame")
	HudRows.Name = "HudRows"
	HudRows.Size = UDim2.new(1, -20, 0, 150)
	HudRows.Position = UDim2.new(0, 12, 0, 78)
	HudRows.BackgroundTransparency = 1
	HudRows.BorderSizePixel = 0
	HudRows.ScrollBarThickness = 2
	HudRows.AutomaticCanvasSize = Enum.AutomaticSize.Y
	HudRows.ZIndex = 9552
	HudRows.Parent = HudPanel
	local lay = Instance.new("UIListLayout")
	lay.SortOrder = Enum.SortOrder.LayoutOrder
	lay.Padding = UDim.new(0, 4)
	lay.Parent = HudRows

	makeDraggable(HudPanel, nil, nil, 0)
	syncHudBtns()
	hudReflow()
end

local function buildKillScriptButton()
	local kbtn = Instance.new("TextButton")
	kbtn.Name = "KillScriptBtn"
	kbtn.Size = UDim2.new(0, 92, 0, 30)
	kbtn.Position = UDim2.new(0, 12, 0, 96)
	kbtn.BackgroundColor3 = Color3.fromRGB(200, 40, 40)
	kbtn.Text = "关闭脚本"
	...
	local cc = Instance.new("UICorner"); cc.CornerRadius = UDim.new(0, 10); cc.Parent = kbtn
	createGrayStroke(kbtn, 1.5)
	kbtn.MouseButton1Click:Connect(function()
		HudDead = true
		pcall(KillEverything)
	end)
end

local function hudUpdater()
	if States.FeatureHud.Enabled then
		if Conns.FeatureHud then return end
		if HudPanel then HudPanel.Visible = true end
		HudDead = false
		hudRefresh()
		hudReflow()
		if not HudGradThread then
			HudGradThread = task.spawn(hudGradLoop)
		end
		Conns.FeatureHud = RunService.RenderStepped:Connect(function() end)
	else
		unbind("FeatureHud")
		HudDead = true
		if HudPanel then HudPanel.Visible = false end
	end
end
Updaters.FeatureHud = hudUpdater
...
KillEverything = function()
	...
	for k in pairs(States) do
		local s = States[k]
		if type(s) == "table" and s.Enabled ~= nil then s.Enabled = false end
	end
	for _, u in pairs(Updaters) do pcall(u) end
	for k in pairs(Conns) do unbind(k) end
	pcall(function() clearRenderCache() end)
	pcall(function() if BeamFolder then BeamFolder:Destroy() end end)
	for _, c in ipairs(ScreenGui:GetChildren()) do
		pcall(function() c:Destroy() end)
	end
end

-- 注册HUD更新入口(setFeatureState会调用它)
UpdateFeatureHud = function(key, enabled)
	if not HudPanel then return end
	local name, ix = hudFeatureName(key)
	if not name then return end
	if enabled then
		hudAddChip(key, name, ix)
	else
		hudRemoveChip(key)
	end
end
		...
		Conns.AdvESP = RunService.RenderStepped:Connect(function()
			...
			for _, t in ipairs(targets) do
				...
				if States.AdvancedESP.ShowBox and height >= 5 then
					...
					local thickness = (States.AdvancedESP.BoxThickness or 1) * 0.25

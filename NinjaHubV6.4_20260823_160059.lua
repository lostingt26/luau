...
local States = {
	...
	AimbotV2 = {Enabled = false, AimPlayer = true, AimNpc = false, AimOther = false, AimPart = "Head", CircleSize = 150, AimSpeed = 0.3, WallCheck = false, TeamCheck = false, AliveCheck = true, Smooth = true, Predict = false, CustomTargets = {}, MaxDistance = 0},
	...
	-- V6.5 新功能
	BigSpin = {Enabled = false, Value = 500},
	God = {Enabled = false},
	FeatureHUD = {Enabled = false, AniDir = "Down", Style = "Line", BG = true, Bold = false, Colors = {"Cyan", "Violet", "Orange"}, Items = {}},
}
...
-- V6.5: 关闭脚本 - 清除全部UI并停止所有功能
local FeatureHUDFrame
local function killScript()
	pcall(function()
		for name in pairs(Conns) do unbind(name) end
		for _, st in pairs(States) do
			if type(st) == "table" then st.Enabled = false end
		end
		ShortcutButtons = {}
		ToggleRefreshers = {}
		FeatureRows = {}
		Gui = {}
		if FeatureHUDFrame then pcall(function() FeatureHUDFrame:Destroy() end); FeatureHUDFrame = nil end
...
-- ============================================
-- V6.5: 功能列表HUD
-- 显示当前已开启功能, 可拖动/调位, 渐变配色/线条样式/动画方向
-- ============================================
local HUDRows = {}
local HUDScroll, HUDList
local HUD_ROW_H = 27
local HUD_GAP = 4
local HUD_VH = 170

local HUDColorPalette = {
	Cyan = Color3.fromRGB(30, 200, 255),
	Red = Color3.fromRGB(255, 70, 70),
	Violet = Color3.fromRGB(150, 70, 255),
	Orange = Color3.fromRGB(255, 170, 30),
	Green = Color3.fromRGB(70, 230, 130),
	Pink = Color3.fromRGB(255, 90, 200),
}

local function hudBuildSequence(keys)
	local ordered = {}
	for _, k in ipairs(keys) do
		if HUDColorPalette[k] then table.insert(ordered, HUDColorPalette[k]) end
	end
	if #ordered == 0 then return ColorSequence.new(Color3.fromRGB(90, 120, 255), Color3.fromRGB(90, 120, 255)) end
	if #ordered == 1 then return ColorSequence.new(ordered[1], ordered[1]) end
	local kp = {}
	local n = #ordered
	for i, col in ipairs(ordered) do
		table.insert(kp, ColorSequenceKeypoint.new((i - 1) / (n - 1), col))
	end
	return ColorSequence.new(kp)
end

local function hudMakeRow(name)
	-- 行: 一个可移动的frame, 内含文本 + 两条线条(上下)
	local row = Instance.new("Frame")
	row.BackgroundTransparency = 1
	row.BorderSizePixel = 0
	row.Size = UDim2.new(1, -6, 0, HUD_ROW_H)
	row.Parent = HUDList
	...
	lbl.ZIndex = 2
	lbl.Parent = row
	local lineT = Instance.new("Frame")
	lineT.Name = "HUDLineTop"
	lineT.Size = UDim2.new(1, 0, 0, 2)
	lineT.Position = UDim2.new(0, 0, 0, 0)
	lineT.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	lineT.BackgroundTransparency = 0.5
	lineT.BorderSizePixel = 0
	lineT.Visible = false
	lineT.Parent = row
	local lineB = Instance.new("Frame")
	lineB.Name = "HUDLineBot"
	lineB.Size = UDim2.new(1, 0, 0, 2)
	lineB.Position = UDim2.new(0, 0, 1, -1)
	lineB.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	lineB.BackgroundTransparency = 0.5
	lineB.BorderSizePixel = 0
	lineB.Visible = false
	lineB.Parent = row
	return row, lbl, lineT, lineB
end

local function hudApplyRowStyle(name)
	local e = HUDRows[name]
	if not e or not e.Row or not e.Row.Parent then return end
	local style = States.FeatureHUD.Style or "Line"
	e.Row.BackgroundTransparency = States.FeatureHUD.BG and 0.25 or 1
	if e.Lbl then
		e.Lbl.Font = States.FeatureHUD.Bold and Enum.Font.GothamBold or Enum.Font.Gotham
	end
	local col = getPartColor(name .. "hudl")
	e.LineT.Visible = style == "Double"
	e.LineB.Visible = style == "Line" or style == "Double"
	e.LineT.BackgroundColor3 = col
	e.LineB.BackgroundColor3 = col
end

-- 刷新HUD列表: 丝滑插入 / 移除 / 重排
local function refreshFeatureHUDRows()
	if not FeatureHUDFrame or not HUDList or not FeatureHUDFrame.Parent then return end
	local names = {}
	for _, feat in ipairs(Features) do
		local st = States[feat.Key]
		if st and type(st) == "table" and st.Enabled then
			table.insert(names, feat.Name)
		end
	end
	-- 移除不再存在 / 新增的行
	local newSet = {}
	for _, n in ipairs(names) do newSet[n] = true end
	for name, e in pairs(HUDRows) do
		if not newSet[name] then
			if e.Row then pcall(function() e.Row:Destroy() end) end
			HUDRows[name] = nil
		end
	end
	for i, name in ipairs(names) do
		if not HUDRows[name] then
			local row, lbl, lineT, lineB = hudMakeRow(name)
			HUDRows[name] = {Row = row, Lbl = lbl, LineT = lineT, LineB = lineB}
			-- 丝滑插入: 从动画方向的一侧滑入
			local dir = States.FeatureHUD.AniDir or "Down"
			local fromY
			if dir == "Down" then fromY = -HUD_ROW_H - 8
			elseif dir == "Up" then fromY = HUD_VH + 8
			else fromY = (i % 2 == 0) and -HUD_ROW_H - 8 or HUD_VH + 8 end
			row.Position = UDim2.new(0, 0, 0, fromY)
			row.Visible = true
			tween(row, {Position = UDim2.new(0, 0, 0, (i - 1) * (HUD_ROW_H + HUD_GAP))}, TweenSlide)
			hudApplyRowStyle(name)
		end
	end
	-- 平滑重排已有行
	local y = 0
	for i, name in ipairs(names) do
		local e = HUDRows[name]
		if e then
			local targetY = (i - 1) * (HUD_ROW_H + HUD_GAP)
			tween(e.Row, {Position = UDim2.new(0, 0, 0, targetY)}, TweenSlide)
			hudApplyRowStyle(name)
			y = targetY + HUD_ROW_H
		end
	end
	if HUDList then
		HUDList.Size = UDim2.new(1, 0, 0, y)
	end
	if HUDScroll then
		HUDScroll.CanvasSize = UDim2.new(0, 0, 0, math.max(y, HUD_VH))
	end
	-- 渐变背景
	if FeatureHUDFrame then
		local grad = FeatureHUDFrame:FindFirstChildOfClass("UIGradient")
		if grad then grad.Color = hudBuildSequence(States.FeatureHUD.Colors or {"Cyan"}) end
	end
end

local function hudSetDir(dir)
	States.FeatureHUD.AniDir = dir
end
local function hudTogStyle(style)
	States.FeatureHUD.Style = style
	refreshFeatureHUDRows()
end
local function hudTogBG(v)
	States.FeatureHUD.BG = v
	refreshFeatureHUDRows()
end
local function hudTogBold(v)
	States.FeatureHUD.Bold = v
	refreshFeatureHUDRows()
end
local function hudToggleColor(key)
	local list = States.FeatureHUD.Colors
	local found = false
	for _, k in ipairs(list) do if k == key then found = true end end
	if found then
...
local function buildFeatureHUDFrame()
	FeatureHUDFrame = Instance.new("Frame")
	FeatureHUDFrame.Name = "FeatureHUD"
	FeatureHUDFrame.Size = UDim2.new(0, 250, 0, HUD_VH + 8)
	FeatureHUDFrame.Position = UDim2.new(0.5, -125, 0.5, -90)
	FeatureHUDFrame.BackgroundColor3 = Color3.fromRGB(14, 10, 38)
	FeatureHUDFrame.BackgroundTransparency = 0.25
	FeatureHUDFrame.BorderSizePixel = 0
	FeatureHUDFrame.Visible = false
	FeatureHUDFrame.ZIndex = 9680
	FeatureHUDFrame.Parent = ScreenGui
	local hudGrad = Instance.new("UIGradient")
	hudGrad.Rotation = 90
	hudGrad.Parent = FeatureHUDFrame
	refreshFeatureHUDRows()
	local hudC = Instance.new("UICorner"); hudC.CornerRadius = UDim.new(0, 14); hudC.Parent = FeatureHUDFrame
	createGrayStroke(FeatureHUDFrame, 2)

	local hudTitle = Instance.new("TextLabel")
	hudTitle.Size = UDim2.new(1, -24, 0, 24)
	hudTitle.Position = UDim2.new(0, 8, 0, 3)
	hudTitle.BackgroundTransparency = 1
	hudTitle.Text = "📋 已开启功能"
	hudTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
	hudTitle.TextSize = 12
	hudTitle.Font = Enum.Font.GothamBold
	hudTitle.Parent = FeatureHUDFrame

	local hudClose = Instance.new("TextButton")
	hudClose.Size = UDim2.new(0, 22, 0, 22)
	hudClose.Position = UDim2.new(1, -28, 0, 4)
	hudClose.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
	hudClose.Text = "×"
	hudClose.TextColor3 = Color3.fromRGB(255, 255, 255)
	hudClose.TextSize = 13
	hudClose.Font = Enum.Font.GothamBold
	hudClose.Parent = FeatureHUDFrame
	local hcC = Instance.new("UICorner"); hcC.CornerRadius = UDim.new(1, 0); hcC.Parent = hudClose
	hudClose.MouseButton1Click:Connect(function()
		FeatureHUDFrame.Visible = false
		States.FeatureHUD.Enabled = false
		if ToggleRefreshers.FeatureHUD then pcall(ToggleRefreshers.FeatureHUD, false) end
	end)

	HUDScroll = Instance.new("ScrollingFrame")
	HUDScroll.Size = UDim2.new(1, -16, 0, HUD_VH)
	HUDScroll.Position = UDim2.new(0, 8, 0, 28)
	HUDScroll.BackgroundTransparency = 1
	HUDScroll.ScrollBarThickness = 3
	HUDScroll.ZIndex = 9681
	HUDScroll.Parent = FeatureHUDFrame
	HUDScroll.CanvasSize = UDim2.new(0, 0, 0, HUD_VH)
	HUDList = Instance.new("Frame")
	HUDList.Size = UDim2.new(1, 0, 0, 0)
	HUDList.BackgroundTransparency = 1
	HUDList.Parent = HUDScroll

	-- 初始重建
	refreshFeatureHUDRows()
	makeDraggable(FeatureHUDFrame)
	raiseZIndex(FeatureHUDFrame, 9682)
end
...
Updaters.FeatureHUD = function()
	if States.FeatureHUD.Enabled then
		...
		if FeatureHUDFrame then
			FeatureHUDFrame.Visible = true
			refreshFeatureHUDRows()
		end
		Conns.FeatureHUD = RunService.Heartbeat:Connect(function()
			if not States.FeatureHUD.Enabled then return end
			if not FeatureHUDFrame then return end
			FeatureHUDFrame.Visible = true
		end)
	else
		unbind("FeatureHUD")
		if FeatureHUDFrame then FeatureHUDFrame.Visible = false end
	end
end
GuisRegistry = nil
...
local function refreshCustomAimList()
	...
	local function listToggle(t, v)
		local out = {}
		for _, x in ipairs(t) do if x ~= v then table.insert(out, x) end end
		if #out == #t then table.insert(out, v) end
		return out
	end
	local function addRow(char, name)
		...
		local function paintRow()
			local sel = listHas(States.AimbotV2.CustomTargets, char)
			row.BackgroundColor3 = sel and Color3.fromRGB(0,150,80) or C.BtnDark
			row.Text = name .. (sel and " ✔" or "")
		end
		paintRow()
		row.MouseButton1Click:Connect(function()
			States.AimbotV2.CustomTargets = listToggle(States.AimbotV2.CustomTargets, char)
			paintRow()
		end)
	end
	for _, e in ipairs(TargetCache.Players) do
...
-- V6.5: 全局基础线宽, 相比 V6.4 再细一半(0.05 -> 0.025)
local BEAM_W = 0.025
...
local function getAdorn(pool, color)
	...
	adorn.Color3 = color
	adorn.ZIndex = 0
	pcall(function() adorn.LineThickness = 0.1 end)
...
-- V6.5: 3D框只渲染棱线(12条棱), 不再填充面. 复用 draw3DLine/getBeam, 不改变3d线绘制逻辑
local function drawBox3D(pool, char, color)
	...
	for _, e in ipairs(edges) do
		draw3DLine(pool, vertices[e[1]], vertices[e[2]], color, BEAM_W)
	end
	return true
end
...
local function drawHLine(pool, parent, x, y, w, color, t)
	...
	line.Size = UDim2.new(0, w, 0, t or 0.75)
...
local function drawVLine(pool, parent, x, y, h, color, t)
	local line = getFromPool(pool, parent)
	line.AnchorPoint = Vector2.new(0, 0)
	line.Size = UDim2.new(0, t or 0.75, 0, h)
	line.Position = UDim2.new(0, x, 0, y)
	line.Rotation = 0
	line.BackgroundColor3 = color
	line.Parent = parent
...
local Features = {
	...
	{Cat=6, Name="功能HUD", Key="FeatureHUD", HasDropdown=true},
			...
			local ok = pcall(function()
				...
				if isBtn then
					if feat.Key == "CloseScript" then
						-- V6.5: 关闭脚本按钮
						local btn = createButton(row, feat.Key, UDim2.new(0, 120, 0, 28), UDim2.new(0, 32, 0.5, -14), Color3.fromRGB(200, 40, 40), "🛑 关闭脚本")
						btn.TextSize = 12
						btn.MouseButton1Click:Connect(function() killScript() end)
					else
						-- V6.2: 远程互动按钮
				...
				elseif isDrop then
					...
					elseif feat.Key == "FeatureHUD" then
						-- V6.5: 功能列表HUD 外观与动画配置
						local hudHint = Instance.new("TextLabel")
						hudHint.Size = UDim2.new(1,0,0,20); hudHint.BackgroundTransparency = 1
						hudHint.Text = "📋 功能HUD 外观与动画"
						hudHint.TextColor3 = Color3.fromRGB(220,220,255); hudHint.TextSize = 11
						hudHint.Font = Enum.Font.Gotham; hudHint.Parent = dropContent

						-- 线条样式: 单文本 / 线条 / 双线
						local hudStyleLabel = Instance.new("TextLabel")
						hudStyleLabel.Size = UDim2.new(1,0,0,20); hudStyleLabel.BackgroundTransparency = 1
						hudStyleLabel.Text = "线条样式"
						hudStyleLabel.TextColor3 = Color3.fromRGB(220,220,255); hudStyleLabel.TextSize = 11
						hudStyleLabel.Font = Enum.Font.Gotham; hudStyleLabel.Parent = dropContent
						local hudStyleRow = createBtnRow(dropContent, 24)
						local hudS1 = createButton(hudStyleRow, "HUDText", UDim2.new(0.32,0,0,22), UDim2.new(), C.BtnDark, "单文本")
						local hudS2 = createButton(hudStyleRow, "HUDLine", UDim2.new(0.32,0,0,22), UDim2.new(0.34,0,0,0), C.BtnDark, "线条")
						local hudS3 = createButton(hudStyleRow, "HUDDouble", UDim2.new(0.32,0,0,22), UDim2.new(0.68,0,0,0), C.BtnDark, "双线")
						hudS1.TextSize = 10; hudS2.TextSize = 10; hudS3.TextSize = 10
						local function hudStyleUpdate()
							local s = States.FeatureHUD.Style or "Line"
							hudS1.BackgroundColor3 = s == "Text" and Color3.fromRGB(0,150,80) or C.BtnDark
							hudS2.BackgroundColor3 = s == "Line" and Color3.fromRGB(0,150,80) or C.BtnDark
							hudS3.BackgroundColor3 = s == "Double" and Color3.fromRGB(0,150,80) or C.BtnDark
						end
						hudS1.MouseButton1Click:Connect(function() hudTogStyle("Text"); hudStyleUpdate() end)
						hudS2.MouseButton1Click:Connect(function() hudTogStyle("Line"); hudStyleUpdate() end)
						hudS3.MouseButton1Click:Connect(function() hudTogStyle("Double"); hudStyleUpdate() end)
						hudStyleUpdate()

						-- 动画方向: 正向 / 反向 / 双向
						local hudDirLabel = Instance.new("TextLabel")
						hudDirLabel.Size = UDim2.new(1,0,0,20); hudDirLabel.BackgroundTransparency = 1
						hudDirLabel.Text = "动画方向"
						hudDirLabel.TextColor3 = Color3.fromRGB(220,220,255); hudDirLabel.TextSize = 11
						hudDirLabel.Font = Enum.Font.Gotham; hudDirLabel.Parent = dropContent
						local hudDirRow = createBtnRow(dropContent, 24)
						local hudD1 = createButton(hudDirRow, "HUDFwd", UDim2.new(0.32,0,0,22), UDim2.new(), C.BtnDark, "正向")
						local hudD2 = createButton(hudDirRow, "HUDRev", UDim2.new(0.32,0,0,22), UDim2.new(0.34,0,0,0), C.BtnDark, "反向")
						local hudD3 = createButton(hudDirRow, "HUDBoth", UDim2.new(0.32,0,0,22), UDim2.new(0.68,0,0,0), C.BtnDark, "双向")
						hudD1.TextSize = 10; hudD2.TextSize = 10; hudD3.TextSize = 10
						local function hudDirUpdate()
							local a = States.FeatureHUD.AniDir or "Down"
							hudD1.BackgroundColor3 = a == "Down" and Color3.fromRGB(0,150,80) or C.BtnDark
							hudD2.BackgroundColor3 = a == "Up" and Color3.fromRGB(0,150,80) or C.BtnDark
							hudD3.BackgroundColor3 = a == "Alternate" and Color3.fromRGB(0,150,80) or C.BtnDark
						end
						hudD1.MouseButton1Click:Connect(function() hudSetDir("Down"); hudDirUpdate() end)
						hudD2.MouseButton1Click:Connect(function() hudSetDir("Up"); hudDirUpdate() end)
						hudD3.MouseButton1Click:Connect(function() hudSetDir("Alternate"); hudDirUpdate() end)
						hudDirUpdate()

						addCheckboxes(dropContent, {
							{"背景颜色", States.FeatureHUD.BG, hudTogBG},
							{"字体加粗", States.FeatureHUD.Bold, hudTogBold},
						})

						-- 渐变颜色(多选)
						local hudColLabel = Instance.new("TextLabel")
						hudColLabel.Size = UDim2.new(1,0,0,20); hudColLabel.BackgroundTransparency = 1
						hudColLabel.Text = "🎨 渐变颜色 (多选)"
						hudColLabel.TextColor3 = Color3.fromRGB(220,220,255); hudColLabel.TextSize = 11
						hudColLabel.Font = Enum.Font.Gotham; hudColLabel.Parent = dropContent
						local hudColorKeys = {"Cyan","Red","Violet","Orange","Green","Pink"}
						local hudColBtns = {}
						local function hudColBuildRow(rowF, startIdx)
							for j = 0, 2 do
								local i = startIdx + j
								local ck = hudColorKeys[i]
								if not ck then break end
								local b = Instance.new("TextButton")
								b.Size = UDim2.new(0.31, 0, 0, 22)
								b.Position = UDim2.new(j * 0.34 + 0.01, 0, 0, 0)
								...
								mark.Text = ck; mark.TextColor3 = Color3.fromRGB(255,255,255)
								mark.TextSize = 9; mark.Font = Enum.Font.Gotham
								mark.Parent = b
								hudColBtns[ck] = b
								b.MouseButton1Click:Connect(function()
									hudToggleColor(ck)
						...
						local function hudColUpdate()
							for ck, b in pairs(hudColBtns) do
								local on = false
								for _, k in ipairs(States.FeatureHUD.Colors) do if k == ck then on = true end end
								b.BackgroundColor3 = on and HUDColorPalette[ck] or Color3.fromRGB(58,42,116)
							end
						end
						local hudColRow1 = createBtnRow(dropContent, 26)
						local hudColRow2 = createBtnRow(dropContent, 26)
						hudColBuildRow(hudColRow1, 1)
						hudColBuildRow(hudColRow2, 4)
						hudColUpdate()
...
-- ============================================
-- V6.5: 大陀螺 - 只旋转身体, 不改视角/改动方向, 纯视觉特效, 速度可调
-- ============================================
Updaters.BigSpin = function()
	if States.BigSpin.Enabled then
		if Conns.BigSpin then return end
		Conns.BigSpin = RunService.RenderStepped:Connect(function()
			if not States.BigSpin.Enabled or not hrp then return end
			local speed = States.BigSpin.Value or 500
			local d = math.rad(speed * task.wait())
			hrp.CFrame = hrp.CFrame * CFrame.Angles(0, d, 0)
		end)
	else
		unbind("BigSpin")
	end
...
Updaters.AutoFire = function()
	if States.AutoFire.Enabled then
		...
		Conns.AutoFire = RunService.Heartbeat:Connect(function()
			...
			updateTargetCache()
			local targets = {}
			local cts = States.AimbotV2.CustomTargets
			if type(cts) == "table" and #cts > 0 then
				for _, ct in ipairs(cts) do
					if ct and ct.Parent then table.insert(targets, ct) end
				end
				if #targets == 0 then States.AimbotV2.CustomTargets = {} end
			end
			if #targets == 0 then
		...
		Conns.BoxCreature = RunService.RenderStepped:Connect(function()
			...
			local function drawFor(char, key)
				...
				elseif States.BoxCreature.BoxMode == "3D" then
					drawBox3D(BeamPools.Box, char, getPartColor(key .. "box"))
				else
				...
				if type(cts) == "table" and #cts > 0 then
					...
					for _, ct in ipairs(cts) do
						if ct and ct.Parent then
							...
							table.insert(alive, {Obj = ct, Hum = hum, Hrp = hrp2})
						end
					end
					if #alive > 0 then
						States.AimbotV2.CustomTargets = alive
						table.move(alive, 1, #alive, 1, targets)
					else
						States.AimbotV2.CustomTargets = {}
					end
				end

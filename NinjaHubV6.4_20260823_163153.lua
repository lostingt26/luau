...
do
	...
	-- 渐变配色预置
	local Palettes = {
		rainbow = {Color3.fromHSV(0,1,1), Color3.fromHSV(0.16,1,1), Color3.fromHSV(0.33,1,1), Color3.fromHSV(0.5,1,1), Color3.fromHSV(0.66,1,1), Color3.fromHSV(0.83,1,1), Color3.fromRGB(230,60,255)},
	...
	local function rowColor(idx, count)
		local cols = paletteColors()
		local t = count <= 1 and 0 or (idx - 1) / (count - 1)
		local seg = t * (#cols - 1)
		local i0 = math.floor(seg + 1e-5)
		local i1 = math.min(i0 + 1, #cols - 1)
		local f = seg - i0
		return cols[i0 + 1]:Lerp(cols[i1 + 1], f)
	end

	-- 固定位置: 每个功能名对应 Features 表中的固定序号
	local FixedOrder = {}
	local nameOf = {}
	...
	local function getEnabledOrder()
		local list = {}
		for key, state in pairs(States) do
			if type(state) == "table" and state.Enabled and not isSkipped(key) then
				table.insert(list, key)
			end
		end
		table.sort(list, function(a, b) return FixedOrder[a] < FixedOrder[b] end)
		return list
	end
	local function sigOf(list) return table.concat(list, "|") end
	...
	local function defaultAnchor()
		local d = States.FeatureHUD.Direction or "forward"
		if d == "reverse" then
			return Vector2.new(0.5, 1), UDim2.new(0.5, 0, 1, -150)
		elseif d == "both" then
			return Vector2.new(0.5, 0.5), UDim2.new(0.5, 0, 0.5, -60)
		end
		return Vector2.new(0.5, 0), UDim2.new(0.5, 0, 0.2, -8)
	end
	...
	local function buildFrame()
		...
		local anchor, defPos = defaultAnchor()
		fr.AnchorPoint = anchor
		local s = States.FeatureHUD
		if s.Pos then
			fr.Position = UDim2.new(s.Pos[1], s.Pos[2], s.Pos[3], s.Pos[4])
		else
			fr.Position = defPos
		end
		fr.Size = UDim2.new(0, F.W, 0, ROW_H)
		fr.Parent = ScreenGui
		F.Frame = fr
		makeDraggable(fr, nil, function()
			local p = fr.Position
			States.FeatureHUD.Pos = {p.X.Scale, p.X.Offset, p.Y.Scale, p.Y.Offset}
		end)
		return fr
	end
	...
	local function makeRow(key)
		...
		bg.BackgroundColor3 = Color3.fromRGB(12, 10, 28)
		bg.BorderSizePixel = 0
		bg.ZIndex = 9804
		bg.Parent = row
		local bgC = Instance.new("UICorner"); bgC.CornerRadius = UDim.new(0, 9); bgC.Parent = bg
		createGrayStroke(bg, 1)

		local barL = Instance.new("Frame")
		barL.Size = UDim2.new(0, 3, 0, 14)
		barL.Position = UDim2.new(0, 5, 0.5, -7)
		barL.BackgroundColor3 = Color3.fromRGB(255,255,255)
		barL.BorderSizePixel = 0
		barL.ZIndex = 9805
		barL.Parent = row
		local barLC = Instance.new("UICorner"); barLC.CornerRadius = UDim.new(0,2); barLC.Parent = barL

		local barR = Instance.new("Frame")
		barR.Size = UDim2.new(0, 3, 0, 14)
		barR.Position = UDim2.new(1, -8, 0.5, -7)
		barR.BackgroundColor3 = Color3.fromRGB(255,255,255)
		barR.BorderSizePixel = 0
		barR.ZIndex = 9805
		barR.Parent = row
		local barRC = Instance.new("UICorner"); barRC.CornerRadius = UDim.new(0,2); barRC.Parent = barR

		local idx = Instance.new("TextLabel")
		idx.Size = UDim2.new(0, 24, 1, 0)
		idx.Position = UDim2.new(0, 13, 0, 0)
		idx.BackgroundTransparency = 1
		idx.Text = ""
		idx.TextColor3 = Color3.fromRGB(255,255,255)
		idx.TextSize = 11
		idx.Font = Enum.Font.GothamBold
		idx.TextXAlignment = Enum.TextXAlignment.Right
		idx.ZIndex = 9806
		idx.Parent = row
		...
		local res = {bg = bg, barL = barL, barR = barR, idx = idx, name = name}
		row.Resources = res
		F.Rows[key] = row
		return row, res
	end

	local function setRowStyle(res, color, number)
		local s = States.FeatureHUD
		local mode = s.Mode or "line"
		res.bg.BackgroundTransparency = s.ShowBg and 0.5 or 1
		res.bg.Visible = s.ShowBg
		res.name.Font = s.Bold and Enum.Font.GothamBold or Enum.Font.Gotham
		res.name.TextColor3 = color
		res.idx.Text = tostring(number)
		res.idx.TextColor3 = color
		res.barL.BackgroundColor3 = color
		res.barR.BackgroundColor3 = color
		res.barL.Visible = (mode == "line" or mode == "dline")
		res.barR.Visible = (mode == "dline")
	end

	local function placeEntry(row)
		local d = States.FeatureHUD.Direction or "forward"
		local sc = Instance.new("UIScale")
		sc.Scale = 0.7
		sc.Parent = row
		row.Resources.name.Transparency = 1
		if d == "reverse" then
			row.Position = UDim2.new(0, 0, 0, -50)
		elseif d == "both" then
			row.Position = UDim2.new(0, 0, 0, 0)
		else
			row.Position = UDim2.new(0, 0, 0, 900)
		end
		return sc
	end
	...
	local function removeRow(key)
		...
		F.Rows[key] = nil
		pcall(function()
			tween(row, {Position = UDim2.new(0, 0, 0, -30)}, TweenFast)
			tween(row.Resources.name, {Transparency = 1}, TweenFast)
			local sc = row:FindFirstChildOfClass("UIScale")
			if sc then tween(sc, {Scale = 0.4}, TweenFast) end
	...
	local function layout()
		if not F.Frame or not F.Frame.Parent then return end
		local list = getEnabledOrder()
		local n = #list
		local totalH = n * (ROW_H + GAP) + PAD * 2
		if totalH < ROW_H then totalH = ROW_H end
		tween(F.Frame, {Size = UDim2.new(0, F.W, 0, totalH)}, TweenSmooth)
		local d = States.FeatureHUD.Direction or "forward"
		for i, key in ipairs(list) do
			local y = PAD + (i - 1) * (ROW_H + GAP)
			local row = F.Rows[key]
			if not row then
				local _, res = makeRow(key)
				row = F.Rows[key]
				local sc = placeEntry(row)
				setRowStyle(res, rowColor(i, n), i)
				if d == "both" then
					-- 双向: 从中心放大淡入
					tween(row, {Position = UDim2.new(0, 0, 0, y)}, TweenSmooth)
					pcall(function() if sc then tween(sc, {Scale = 1}, TweenScalePop) end end)
					tween(res.name, {Transparency = 0}, TweenSmooth)
				else
					tween(row, {Position = UDim2.new(0, 0, 0, y)}, TweenSlide)
					pcall(function() if sc then tween(sc, {Scale = 1}, TweenSmooth) end end)
					tween(res.name, {Transparency = 0}, TweenSlide)
				end
			else
				local cur = row.Position.Y.Offset
				if math.abs(cur - y) > 1 then
					tween(row, {Position = UDim2.new(0, 0, 0, y)}, TweenSlide)
				else
					row.Position = UDim2.new(0, 0, 0, y)
				end
				setRowStyle(row.Resources, rowColor(i, n), i)
			end
		end
	end
	...
	local function sync()
		...
		for key in pairs(F.Rows) do
			if not inList(list, key) then removeRow(key) end
		end
		layout()
	end

	-- 供下拉面板调用: 立即应用样式变化(方向/模式/配色/粗细/背景/加粗)
	F.reapply = function()
		...
		if F.Frame and F.Frame.Parent then
			local wantAnchor, _ = defaultAnchor()
			if F.Frame.AnchorPoint ~= wantAnchor then
	...
	F.resetPos = function()
		States.FeatureHUD.Pos = nil
		if F.Frame and F.Frame.Parent then
			local _, defPos = defaultAnchor()
			tween(F.Frame, {Position = defPos}, TweenSmooth)
			makeDraggable(F.Frame, nil, function()
				local p = F.Frame.Position
				States.FeatureHUD.Pos = {p.X.Scale, p.X.Offset, p.Y.Scale, p.Y.Offset}
			end)
		end
	end
	...
	Updaters.FeatureHUD = function()
		if States.FeatureHUD.Enabled then
			...
			buildFrame()
			if F.Frame then F.Frame.Visible = true end
			sync()
			Conns.FeatureHUD = RunService.Heartbeat:Connect(function()
				if not States.FeatureHUD.Enabled then return end
				local list = getEnabledOrder()
				local sig = sigOf(list)
...
Updaters.BigSpin = function()
	if States.BigSpin.Enabled then
		...
		Conns.BigSpin = RunService.RenderStepped:Connect(function(dt)
			...
			if not root then return end
			local spd = States.BigSpin.Speed or 30
			spin = spin + math.rad(spd) * (dt or 0.016) * 3
			-- 仅绕自身竖直轴旋转, 位置不动, 视角与移动方向不受影响
			local cfg = CFrame.new(root.Position)
			root.CFrame = cfg * (root.CFrame - root.Position).Rotation * CFrame.Angles(0, spin, 0)
		end)
	else
		unbind("BigSpin")

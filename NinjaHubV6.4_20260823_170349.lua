...
-- ============================================
do
	--================ 功能列表 HUD (完全重写, 仿 Java Hud) ================
	-- 显示: 右上角, 右对齐, 最长名字在上, 连续流动的彩色渐变, 丝滑滑入/滑出
	-- 触摸: 除顶部小巧拖拽条外全部穿透, 不阻挡游戏交互
	local F = {}
	F.Rows = {}
	F.LastSig = ""
	local ROW_H = 20
	...
	local function buildFrame()
		...
		fr.Size = UDim2.new(0, 200, 0, DRAG)
		...
		fr.ZIndex = 9800
		local s = States.FeatureHUD
		if s.Pos and type(s.Pos) == "table" then
			fr.Position = UDim2.new(s.Pos[1], s.Pos[2], s.Pos[3], s.Pos[4])
		else
			fr.Position = UDim2.new(1, -24, 0, 24)
			s.Pos = {1, -24, 0, 24}
		end
		fr.Visible = false
		fr.Parent = ScreenGui
		F.Frame = fr
		-- 顶部拖拽把手(唯一拦截输入的区域, 其余全部穿透)
		local grip = Instance.new("Frame")
		grip.Name = "NH_HUDDrag"
		grip.Size = UDim2.new(1, 0, 0, DRAG)
	...
	local function makeRow(key, width)
		...
		bg.BackgroundTransparency = s.ShowBg ~= false and 0.2 or 1
		bg.BorderSizePixel = 0
		bg.Active = false
		bg.ZIndex = 9803
		bg.Visible = s.ShowBg ~= false
		bg.Parent = row
		local bgC = Instance.new("UICorner"); bgC.CornerRadius = UDim.new(0, 5); bgC.Parent = bg
		res.bg = bg
		-- 左边线(双线条用)
		local lineL = Instance.new("Frame")
		lineL.Size = UDim2.new(0, 2, 1, 0)
		lineL.Position = UDim2.new(0, 1, 0, 0)
	...
	local function applyStyle(res, color)
		local s = States.FeatureHUD
		local mode = s.Mode or "line"
		res.bg.Visible = s.ShowBg ~= false
		res.bg.BackgroundTransparency = (s.ShowBg ~= false) and 0.2 or 1
		...
		res.lineR.Visible = mode == "line" or mode == "dline"
	end
	...
	local function layout()
		...
		local frameW = wid + PAD_R + 6
		local totalH = DRAG + n * (ROW_H + GAP) + 4
		tween(fr, {Size = UDim2.new(0, frameW, 0, totalH)}, TweenSmooth)
		for i, k in ipairs(list) do
			local y = DRAG + (i - 1) * (ROW_H + GAP)
			local row = F.Rows[k]
			if not row then
				local _, res = makeRow(k, frameW)
				row = F.Rows[k]
				row.Position = UDim2.new(0, 60, 0, y)
				...
				if States.FeatureHUD.ShowBg ~= false then tween(res.bg, {BackgroundTransparency = 0.2}, TweenSmooth) end
			else
	...
	local function removeRow(key)
		...
		pcall(function()
			...
			tween(row, {Position = UDim2.new(0, 70, 0, y)}, TweenSlide)
	...
	F.resetPos = function()
		States.FeatureHUD.Pos = {1, -24, 0, 24}
		if F.Frame and F.Frame.Parent then
			tween(F.Frame, {Position = UDim2.new(1, -24, 0, 24)}, TweenSmooth)

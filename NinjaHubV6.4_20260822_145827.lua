-- ============================================
-- Ninja Hub V6.2 - 超级自动化版
-- 新增: 点击脚本 / 客户端脚本录制回放 / 远程互动 / 多球调节
...
-- ============================================
-- 反作弊绕过层 V6.4 (全pcall保护, 不影响加载)
-- 利用注入器扩展API: newcclosure/getgenv/checkcaller等
-- ============================================
local _nc = nil
pcall(function() _nc = newcclosure end)
local _gge = nil
pcall(function() _gge = getgenv end)
-- cc(): 若注入器可用则用newcclosure包裹回调, 使checkcaller误判为Roblox原生调用
local function cc(fn)
	if _nc and typeof(fn) == "function" then
		local ok, f = pcall(_nc, fn)
		if ok and f then return f end
	end
	return fn
end
-- 环境标记: 放入getgenv避免被GC遍历/反作弊扫描识别注入痕迹
pcall(function()
	if _gge then
		_gge()["NinjaHub"] = _gge()["NinjaHub"] or {}
		_gge()["NinjaHub"].Version = "V6.4"
		_gge()["NinjaHub"].Bot = true
		_gge()["NinjaHub"].Safe = true
	end
end)
-- 隐藏脚本自身来源: 移除getenvinfo里我们能找到的自身脚本名 (尽力而为, 全部pcall)
local function fakeScriptInfo()
	pcall(function()
		local info = getinfo(2) or {}
		if info.source then
			info.source = "=[C]"
			info.linedefined = 0
			info.short_src = "[C]"
...
-- getrawmetatable软限制: 防止反作弊通过__namecall/__index拦截我们的操作
local _grm = nil
pcall(function() _grm = getrawmetatable end)
local _setRO = nil
pcall(function() _setRO = setreadonly end)
pcall(function()
	if _grm and _setRO then
		local mt = _grm(game)
		if mt then pcall(function() _setRO(mt, true) end) end
	end
end)
...
local States = {
	...
	-- V6.4 新功能
	KeyMapper = {Enabled = false, Recording = false, Keys = {}, Selected = nil},
...
-- V6.4: 按键映射系统
-- ============================================
local KeyFlts = {}      -- keyName -> gui 悬浮按钮
local KeyFlOrder = {}   -- 记录创建顺序
local KeyMapperLogRefresh = nil

local function kmKeyName(code)
	if typeof(code) == "EnumItem" and code.EnumType == Enum.KeyCode then
		return tostring(code.Name)
	end
	return nil
end

local function removeKeyFloat(keyName)
	local b = KeyFlts[keyName]
	if b then
		pcall(function() b:Destroy() end)
		KeyFlts[keyName] = nil
		for i, k in ipairs(KeyFlOrder) do
			if k == keyName then table.remove(KeyFlOrder, i); break end
		end
	end
	if KeyMapperLogRefresh then pcall(KeyMapperLogRefresh) end
end

local function kmRemoveAllFloats()
	for k in pairs(KeyFlts) do removeKeyFloat(k) end
end

local function addKeyFloat(keyName, toggleMode)
	if not keyName or #keyName == 0 then return nil end
	if KeyFlts[keyName] then return KeyFlts[keyName] end
	local norm = Color3.fromRGB(130, 60, 170)
	local inv = Color3.fromRGB(0, 255, 90)
	local btn = Instance.new("TextButton")
	btn.Name = "KeyMapFloat_" .. keyName
	btn.Size = UDim2.new(0, 80, 0, 42)
	local n = #KeyFlOrder
	btn.Position = UDim2.new(0.5, -40 + (n % 6) * 84, 0.28, (n % 5) * 56)
	...
	local state = {On = false, Toggle = toggleMode or false}
	local stroke = btn:FindFirstChildOfClass("UIStroke")
	local function apply(on)
		state.On = on
		if on then
			btn.BackgroundColor3 = inv
			if stroke then stroke.Color = Color3.fromRGB(0, 255, 160) end
		else
			btn.BackgroundColor3 = norm
	...
	btn.MouseButton1Up:Connect(function()
		if not state.Toggle then apply(false) end
	end)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			dragConn = UserInputService.InputChanged:Connect(function(ch)
				...
				if ch.UserInputType == Enum.UserInputType.MouseMovement or ch.UserInputType == Enum.UserInputType.Touch then
					local delta = ch.Position - startM
					btn.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
				end
			end)
			endConn = UserInputService.InputEnded:Connect(function(ie)
				if ie.UserInputType == Enum.UserInputType.MouseButton1 or ie.UserInputType == Enum.UserInputType.Touch then
					dragging = false
	...
	closeBtn.Position = UDim2.new(1, -21, 0, -8)
	closeBtn.BackgroundColor3 = Color3.fromRGB(220, 60, 60)
	closeBtn.Text = "×"
	closeBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	closeBtn.TextSize = 11
	closeBtn.Font = Enum.Font.GothamBold
	closeBtn.ZIndex = 9810
	closeBtn.Parent = btn
	local xc = Instance.new("UICorner"); xc.CornerRadius = UDim.new(1, 0); xc.Parent = closeBtn
	closeBtn.MouseButton1Click:Connect(function()
		removeKeyFloat(keyName)
	end)

	KeyFlts[keyName] = btn
	table.insert(KeyFlOrder, keyName)
	return btn
end

-- 全局按键录制监听(仅在按键映射启用且处于录制时记录)
UserInputService.InputBegan:Connect(function(input, gpe)
	if gpe then return end
	local km = States.KeyMapper
	if not km.Enabled or not km.Recording then return end
	if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
	local kn = kmKeyName(input.KeyCode)
	if not kn or kn == "Escape" then return end
	local found = false
	for _, ent in ipairs(km.Keys) do
		if ent.Name == kn then found = true; break end
	end
	if not found then
		table.insert(km.Keys, {Name = kn})
	end
	if KeyMapperLogRefresh then pcall(KeyMapperLogRefresh) end
end)
...
-- ============================================
-- V6.4: 本地MP3模式 (读取本地文件夹, 用于注入器Storage权限)
-- ============================================
local LocalFolderInput = nil
local LocalTabTip = nil
Music.LocalFolder = ""   -- 用户输入的本地文件夹路径
Music.LocalList = {}     -- 扫描到的本地mp3列表: {name=, path=}

-- 列出文件夹下所有 .mp3 文件 (优先listfiles, 兼容io读目录脚本)
local function localListSongs(folder)
	folder = folder or ""
	local found = {}
	if #folder == 0 then return found end
	-- 方法1: 注入器常用 listfiles (Ninja部分版本可用)
	local hasList = pcall(function()
		local fs = listfiles(folder)
		for _, f in ipairs(fs) do
			local lm = tostring(f):lower()
			if lm:find("%.mp3$") then
				table.insert(found, {name = tostring(f):match("([^/\\]+)$") or tostring(f), path = tostring(f)})
			end
		end
	end)
	-- 方法2: io 逐文件探测 (仅能根据用户给的名字, 不能列目录, 靠mantcount提示)
	if #found == 0 then
		pcall(function()
			if isfolder then
...
-- 播放本地mp3: 优先playfile, 后备getcustomasset+Sound, 再后备playsound
local LocalSound = nil
local function playLocalMp3(path)
	...
	-- 方式2: getcustomasset + Sound
	pcall(function()
		if LocalSound and LocalSound.Parent then LocalSound:Destroy() end
		if getcustomasset then
			local id = tostring(getcustomasset(path)):gsub("^rbxasset://", "")
			LocalSound = Instance.new("Sound")
			LocalSound.SoundId = "rbxasset://" .. id
			LocalSound.Parent = player.Character or playerGui
			LocalSound:Play()
			Music.Playing = true
			updateMusicPlayBtn()
			played = true
	...
	-- 方式3: playsound
	played = pcall(function()
		if playsound then playsound(path) end
	end)
	if played then
		Music.Playing = true
		updateMusicPlayBtn()
...
local function stopLocalMp3()
	Music.Playing = false
	updateMusicPlayBtn()
	pcall(function() if LocalSound and LocalSound.Parent then LocalSound:Destroy() end end)
	pcall(function() if stopfile then stopfile() end end)
end

-- 本地Tab: 扫描指定文件夹
local function refreshLocalMusic()
	folder = Music.LocalFolder or ""
	if #folder == 0 then
		Music.LocalList = {}
		if LocalTabTip then LocalTabTip.Text = "⌨ 请先输入本地文件夹路径" end
		renderMusicList()
		return
	end
	Music.LocalList = localListSongs(folder)
	if #Music.LocalList > 0 then
		if LocalTabTip then LocalTabTip.Text = "✅ 找到 " .. #Music.LocalList .. " 首本地MP3" end
	else
		if LocalTabTip then LocalTabTip.Text = "⚠ 未找到MP3(检查路径或listfiles支持)" end
	end
	renderMusicList()
end

-- V6.4: 通用mp3写入(优先writefile, 后备io.open)
local function writeMp3Bytes(name, body)
	local ok = pcall(function() writefile(name, body) end)
...
-- V6.4: 通用mp3播放后端(repeat 尝试 playfile / getcustomasset+Sound / playsound)
local function playMp3Backend(name, path)
	local p = path or name
	...
	if ok then return true end
	ok = pcall(function()
		if not getcustomasset then error("no getcustomasset") end
		if LocalSound and LocalSound.Parent then LocalSound:Destroy() end
		local id = tostring(getcustomasset(p)):gsub("^rbxasset://", "")
		LocalSound = Instance.new("Sound")
		LocalSound.SoundId = "rbxasset://" .. id
		LocalSound.Parent = player.Character or playerGui
		LocalSound:Play()
	...
	if ok then return true end
	ok = pcall(function()
		if playsound then playsound(p) end
	end)
	return ok
end
...
local function playSongAt(idx)
	...
	if s.Local then
		-- 本地mp3
		task.spawn(function()
			local ok = playLocalMp3(s.path)
			if ok then
				musicToast("▶ 播放(本地): " .. s.name)
				if MusicTimer then task.cancel(MusicTimer) end
				MusicTimer = task.delay((s.dt or 240000) / 1000 + 1.5, function()
					if Music.Current == s and Music.Playing then
						onMusicEnd()
					end
				end)
			else
				musicToast("⚠ 本地播放失败, 请确认播放器支持")
				if Music.Current == s then onMusicEnd() end
			end
		end)
		return
	end
	task.spawn(function()
		...
		if body then
			local okW = writeMp3Bytes("NHMusic.mp3", body)
			local okP = okW and playMp3Backend("NHMusic.mp3") or false
			if okP then
				Music.Playing = true
				updateMusicPlayBtn()
...
local function setMusicTab(tab)
	...
	if tab == "Search" then
		...
		if Gui.MusicSearchLocalRow then Gui.MusicSearchLocalRow.Visible = false end
		if MusicListFrame then MusicListFrame.Position = UDim2.new(0, 0, 0, 122) end
	else
	...
	elseif tab == "Local" then
		Music.List = Music.LocalList
		if LocalTabTip then LocalTabTip.Text = "⏳ 扫描本地文件夹..." end
		if Gui.MusicSearchLocalRow then Gui.MusicSearchLocalRow.Visible = true end
		if MusicListFrame then MusicListFrame.Position = UDim2.new(0, 0, 0, 182) end
		task.spawn(function() refreshLocalMusic() end)
		refreshLocalButton()
	elseif tab == "Rec" then
		Music.List = {}
		refreshLocalButton()
...
local function buildMusicPanel()
	...
	local tabs = {{"Rec", "🔥 推荐"}, {"Search", "🔍 搜索"}, {"Local", "📂 本地"}, {"Favs", "❤️ 收藏"}}
	for i, t in ipairs(tabs) do
		...
		b.Parent = tabRow
		local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, 8); c.Parent = b
		if Music.Tab == t[1] then b.BackgroundColor3 = Color3.fromRGB(0, 130, 90) end
		b.MouseButton1Click:Connect(function()
			setMusicTab(t[1])
		end)
	end

	-- V6.4: 本地音乐行(文件夹路径 + 扫描 + 提示)
	local localRow = Instance.new("Frame")
	localRow.Size = UDim2.new(1, 0, 0, 56)
	localRow.Position = UDim2.new(0, 0, 0, 120)
	localRow.BackgroundTransparency = 1
	localRow.Visible = false
	localRow.Parent = content
	local localRowLayout = Instance.new("UIListLayout"); localRowLayout.Padding = UDim.new(0, 4); localRowLayout.Parent = localRow
	local folderRow = Instance.new("Frame")
	folderRow.Size = UDim2.new(1, 0, 0, 26); folderRow.BackgroundTransparency = 1; folderRow.Parent = localRow
	local folderInput = Instance.new("TextBox")
	folderInput.Size = UDim2.new(1, -58, 0, 24); folderInput.Position = UDim2.new(0, 0, 0, 1)
	folderInput.BackgroundColor3 = Color3.fromRGB(40, 30, 90)
	folderInput.PlaceholderText = "本地MP3文件夹路径..."
	folderInput.PlaceholderColor3 = Color3.fromRGB(140, 140, 190)
	folderInput.Text = Music.LocalFolder or ""
	folderInput.TextColor3 = Color3.fromRGB(255, 255, 255); folderInput.TextSize = 11; folderInput.Font = Enum.Font.Gotham
	folderInput.Parent = folderRow
	local fic = Instance.new("UICorner"); fic.CornerRadius = UDim.new(0, 8); fic.Parent = folderInput
	local scanBtn = Instance.new("TextButton")
	scanBtn.Size = UDim2.new(0, 54, 0, 24); scanBtn.Position = UDim2.new(1, -54, 0, 1)
	scanBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 110); scanBtn.Text = "📂 扫描"; scanBtn.TextColor3 = Color3.fromRGB(255,255,255)
	scanBtn.TextSize = 11; scanBtn.Font = Enum.Font.GothamBold; scanBtn.Parent = folderRow
	local scc = Instance.new("UICorner"); scc.CornerRadius = UDim.new(0, 8); scc.Parent = scanBtn
	local tipL2 = Instance.new("TextLabel")
	tipL2.Size = UDim2.new(1, 0, 0, 20); tipL2.BackgroundTransparency = 1
	tipL2.Text = "⌨ 输入文件夹路径后点扫描"; tipL2.TextColor3 = Color3.fromRGB(150, 200, 150); tipL2.TextSize = 9
	tipL2.Font = Enum.Font.Gotham; tipL2.TextXAlignment = Enum.TextXAlignment.Left; tipL2.Parent = localRow
	LocalFolderInput = folderInput
	LocalTabTip = tipL2
	function refreshLocalButton()
		if tipL2 then
			local n = #Music.LocalList
			tipL2.Text = (Music.Tab == "Local") and ("本地共 " .. n .. " 首") or ("❤️ 收藏 " .. n .. " 首")
		end
	end
	folderInput.FocusLost:Connect(function()
		Music.LocalFolder = folderInput.Text
	end)
	...
	Gui.MusicSearchLocalRow = localRow
...
local function teleportThrough(target)
	...
	for i = 1, 60 do
		...
		hrp.CFrame = current:Lerp(target, 0.25)
		task.wait(1 / 60)
	end
...
local function buildClickScriptPanel()
	...
	-- V6.4: 小球数量直接增删(修复无法添加小球)
	local ballLabel = Instance.new("TextLabel")
	ballLabel.Size = UDim2.new(0, 52, 0, 22)
	ballLabel.Position = UDim2.new(0, 150, 0, 1)
	...
	ballLabel.Text = "小球" .. tostring(#(Gui.ClickerBalls or {}))
	ballLabel.Parent = repeatRow
	local blc = Instance.new("UICorner"); blc.CornerRadius = UDim.new(0, 6); blc.Parent = ballLabel
	local ballMinus = createButton(repeatRow, "BallMinus", UDim2.new(0, 20, 0, 22), UDim2.new(0, 204, 0, 1), C.Btn, "-")
	local ballPlus = createButton(repeatRow, "BallPlus", UDim2.new(0, 20, 0, 22), UDim2.new(0, 226, 0, 1), C.Btn, "+")
	local function setClickerBallCount(n)
		n = math.clamp(n, 1, 8)
		States.ClickerMulti.BallCount = n
		States.ClickerMulti.Enabled = true
		while #Gui.ClickerBalls < n do
		...
		for i, ball in ipairs(Gui.ClickerBalls) do
			ball.Visible = States.ClickScript.Enabled
			local orderLbl = ball:FindFirstChild("OrderLabel")
			if orderLbl then orderLbl.Text = tostring(i) end
		end
		ballLabel.Text = "小球" .. tostring(#Gui.ClickerBalls)
	end
	ballMinus.MouseButton1Click:Connect(function()
		setClickerBallCount((States.ClickerMulti.BallCount or 4) - 1)
	end)
	ballPlus.MouseButton1Click:Connect(function()
		setClickerBallCount((States.ClickerMulti.BallCount or 4) + 1)
	end)
...
local Features = {
	...
	{Cat=4, Name="按键映射", Key="KeyMapper", HasDropdown=true},
	...
	for _, feat in ipairs(Features) do
		if feat.Cat == CurrentCategory then
			...
			local ok = pcall(function()
				...
				if isBtn then
					-- V6.2: 远程互动按钮
					local btn = createButton(row, feat.Key, UDim2.new(0, 120, 0, 28), UDim2.new(0, 32, 0.5, -14), Color3.fromRGB(90, 65, 160), "🌐 远程互动")
					btn.TextSize = 12
					local grad = Instance.new("UIGradient")
					grad.Color = ColorSequence.new(Color3.fromRGB(255, 50, 150), Color3.fromRGB(50, 110, 255))
					grad.Rotation = 90
					grad.Parent = btn
					btn.MouseButton1Click:Connect(function()
						musicToast("🌐 远程互动中...")
				...
				elseif isDrop then
					...
					elseif feat.Key == "KeyMapper" then
						-- V6.4: 按键映射(记录按键 -> 生成独立悬浮按钮)
						local kmStatus = Instance.new("TextLabel")
						kmStatus.Size = UDim2.new(1, 0, 0, 20); kmStatus.BackgroundTransparency = 1
						kmStatus.Text = States.KeyMapper.Recording and "⏺ 录制中... 请点击按键" or ("⏹ 已记录 " .. #States.KeyMapper.Keys .. " 键")
						kmStatus.TextColor3 = Color3.fromRGB(140, 255, 170); kmStatus.TextSize = 11
						kmStatus.Font = Enum.Font.Gotham; kmStatus.Parent = dropContent

						local kmRecRow = createBtnRow(dropContent, 26)
						local kmRecBtn = createButton(kmRecRow, "KMRec", UDim2.new(0.48, 0, 0, 24), UDim2.new(), C.BtnDark, States.KeyMapper.Recording and "⏹ 停止录制" or "⏺ 开始录制")
						local kmClearBtn = createButton(kmRecRow, "KMClear", UDim2.new(0.48, 0, 0, 24), UDim2.new(0.52, 0, 0, 0), C.BtnDark, "🗑 清空")
						kmRecBtn.TextSize = 11; kmClearBtn.TextSize = 11

						local kmLogTitle = Instance.new("TextLabel")
						kmLogTitle.Size = UDim2.new(1, 0, 0, 18); kmLogTitle.BackgroundTransparency = 1
						kmLogTitle.Text = "📜 按键日志(点击选择):"; kmLogTitle.TextColor3 = Color3.fromRGB(220, 220, 255)
						kmLogTitle.TextSize = 10; kmLogTitle.Font = Enum.Font.GothamBold; kmLogTitle.Parent = dropContent

						local kmLogFrame = Instance.new("ScrollingFrame")
						kmLogFrame.Size = UDim2.new(1, 0, 0, 72)
						kmLogFrame.BackgroundColor3 = Color3.fromRGB(30, 20, 66); kmLogFrame.BackgroundTransparency = 0.4
						kmLogFrame.ScrollBarThickness = 3; kmLogFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
						kmLogFrame.Parent = dropContent
						local kmLogC = Instance.new("UICorner"); kmLogC.CornerRadius = UDim.new(0, 8); kmLogC.Parent = kmLogFrame
						local kmLogLay = Instance.new("UIListLayout"); kmLogLay.Padding = UDim.new(0, 2); kmLogLay.Parent = kmLogFrame

						local kmFloatTitle = Instance.new("TextLabel")
						kmFloatTitle.Size = UDim2.new(1, 0, 0, 18); kmFloatTitle.BackgroundTransparency = 1
						kmFloatTitle.Text = "🎯 悬浮按钮(独立可移动):"; kmFloatTitle.TextColor3 = Color3.fromRGB(220, 220, 255)
						kmFloatTitle.TextSize = 10; kmFloatTitle.Font = Enum.Font.GothamBold; kmFloatTitle.Parent = dropContent

						local kmOptRow = createBtnRow(dropContent, 26)
						local kmAddBtn = createButton(kmOptRow, "KMAdd", UDim2.new(0.48, 0, 0, 24), UDim2.new(), Color3.fromRGB(90, 65, 160), "➕ 添加所选键")
						local kmToggleBtn = createButton(kmOptRow, "KMToggle", UDim2.new(0.48, 0, 0, 24), UDim2.new(0.52, 0, 0, 0), C.BtnDark, "🔘 瞬时模式")
						kmAddBtn.TextSize = 11; kmToggleBtn.TextSize = 11
						local kmToggleOn = false
						kmToggleBtn.MouseButton1Click:Connect(function()
							kmToggleOn = not kmToggleOn
							kmToggleBtn.BackgroundColor3 = kmToggleOn and Color3.fromRGB(0, 150, 80) or C.BtnDark
							kmToggleBtn.Text = kmToggleOn and "🔒 开关模式" or "🔘 瞬时模式"
						end)
						local kmDelAll = createButton(dropContent, "KMDelAll", UDim2.new(1, 0, 0, 24), UDim2.new(), Color3.fromRGB(200, 60, 60), "🗑 移除所有悬浮按钮")
						kmDelAll.TextSize = 11

						local function kmRefresh()
							kmStatus.Text = States.KeyMapper.Recording and "⏺ 录制中... 请点击按键" or ("⏹ 已记录 " .. #States.KeyMapper.Keys .. " 键")
							kmRecBtn.Text = States.KeyMapper.Recording and "⏹ 停止录制" or "⏺ 开始录制"
							for _, c in ipairs(kmLogFrame:GetChildren()) do
								if c:IsA("TextButton") or c:IsA("TextLabel") then c:Destroy() end
							end
							local keys = States.KeyMapper.Keys
							if #keys == 0 then
								local empty = Instance.new("TextLabel")
								empty.Size = UDim2.new(1, 0, 0, 20); empty.BackgroundTransparency = 1
								empty.Text = "暂无记录(点击开始录制)"; empty.TextColor3 = Color3.fromRGB(150, 150, 170)
								empty.TextSize = 9; empty.Font = Enum.Font.Gotham; empty.TextXAlignment = Enum.TextXAlignment.Center
								empty.Parent = kmLogFrame
								return
							end
							for _, ent in ipairs(keys) do
								local sel = (States.KeyMapper.Selected == ent.Name)
								local row = Instance.new("TextButton")
								row.Size = UDim2.new(1, 0, 0, 20)
								row.BackgroundColor3 = sel and Color3.fromRGB(0, 150, 80) or C.BtnDark
								row.BackgroundTransparency = 0.3
								row.Text = ent.Name
								row.TextColor3 = Color3.fromRGB(255, 255, 255)
								row.TextSize = 9; row.Font = Enum.Font.Gotham
								row.Parent = kmLogFrame
								local rcc = Instance.new("UICorner"); rcc.CornerRadius = UDim.new(0, 6); rcc.Parent = row
								row.MouseButton1Click:Connect(function()
									States.KeyMapper.Selected = ent.Name
									kmRefresh()
								end)
							end
						end

						kmRecBtn.MouseButton1Click:Connect(function()
							States.KeyMapper.Recording = not States.KeyMapper.Recording
							kmRefresh()
						end)
						kmClearBtn.MouseButton1Click:Connect(function()
							States.KeyMapper.Keys = {}
							States.KeyMapper.Selected = nil
							kmRefresh()
						end)
						kmAddBtn.MouseButton1Click:Connect(function()
							local sel = States.KeyMapper.Selected
							if not sel then
								kmStatus.Text = "⚠ 请先在日志中选择一个按键"
								return
							end
							addKeyFloat(sel, kmToggleOn)
							kmStatus.Text = "✅ 已添加悬浮按钮: " .. sel .. (kmToggleOn and " (开关)" or " (瞬时)")
						end)
						kmDelAll.MouseButton1Click:Connect(function()
							kmRemoveAllFloats()
							kmStatus.Text = "🧹 已移除全部悬浮按钮"
						end)
						KeyMapperLogRefresh = kmRefresh
						kmRefresh()
	...
	while true do
		...
		for key, state in pairs(States) do
			if type(state) == "table" and state.Enabled and Updaters[key] then
				local skip = key == "AutoSave" or key == "AntiAfk" or key == "ClickerStart" or key == "DynamicIsland" or key == "MusicPlayer" or key == "ClickScript" or key == "KeyMapper" or key == "RemoteInteract"

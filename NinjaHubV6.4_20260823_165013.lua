...
do
	...
	-- 循环渐变采样: t 沿调色板首尾相接循环过渡
	local function gradColorAt(t)
		...
		local i1 = (i0 + 1) % N
		local f = g - math.floor(g)
		return cols[i0 + 1]:Lerp(cols[i1 + 1], f)
	end

	-- 测量文字像素宽度(仅用于排序与整体宽度), 失败时退化为字符数估算
	local TextService = game:GetService("TextService")
	local function currentBold()
		return (States.FeatureHUD.Bold == true)
	end
	local function measurePx(text)
		local font = currentBold() and Enum.Font.GothamBold or Enum.Font.Gotham
		local ok, b = pcall(function()
			return TextService:GetTextBoundsAsync({Text = text, Font = font, TextSize = TEXTSIZE, Width = 0})
		end)
		if ok and type(b) == "userdata" and b and b.X and b.X > 0 then
			return (b.X + 6)
		end
		local n = 0
		pcall(function() n = utf8.len(text, 1, #text) end)
		return (n > 0 and n or #text) * TEXTSIZE + 6
	end

	-- 固定位置: 每个功能名对应 Features 表中的固定序号(用于宽度相同情况下保持顺序稳定)
	local FixedOrder = {}
	local nameOf = {}
	...
	-- 按宽度降序(最长在上, 右对齐统一), 宽度相同按功能表顺序
	local function sortedKeys()
		local list = getEnabledOrder()
		local w = {}
		for _, k in ipairs(list) do w[k] = measurePx(nameOf[k] or k) end
	...
	local function defaultPos()
		return Vector2.new(0.5, 0), UDim2.new(0.5, 0, 0, 150)
	end
	...
	local function buildFrame()
		...
		fr.Active = false
		fr.Selectable = false
		local anchor, defPos = defaultPos()
		fr.AnchorPoint = anchor
		local s = States.FeatureHUD
		if s.Pos then fr.Position = UDim2.new(s.Pos[1], s.Pos[2], s.Pos[3], s.Pos[4])
		else fr.Position = defPos end
		fr.Size = UDim2.new(0, 200, 0, DRAG_H)
		fr.Parent = ScreenGui
		F.Frame = fr
		-- 顶部隐形拖拽条(可整体移动; 仅此条拦截输入, 其余全部触摸穿透)
		local grip = Instance.new("Frame")
		grip.Name = "NH_HUDDrag"
		grip.Size = UDim2.new(1, 0, 0, DRAG_H)
		...
		bar.Size = UDim2.new(0, 46, 0, 4)
		bar.Position = UDim2.new(0.5, 0, 1, -5)
		...
		F.Grip = grip
		makeDraggable(fr, grip, function()
			local p = fr.Position
	...
	local function makeTopLine()
		if F.TopLine and F.TopLine.Parent then return F.TopLine end
		local tl = Instance.new("Frame")
		tl.Name = "NH_HUDTopLine"
		tl.Size = UDim2.new(0, 200, 0, 3)
		tl.Position = UDim2.new(0, 0, 0, DRAG_H - 2)
	...
	local function makeRow(key, width)
		...
		bg.BackgroundTransparency = s.ShowBg and 0.22 or 1
		...
		lineL.Size = UDim2.new(0, 3, 1, 0)
		lineL.Position = UDim2.new(0, 0, 0, 0)
		...
		lineR.Size = UDim2.new(0, 3, 1, 0)
		lineR.Position = UDim2.new(1, -3, 0, 0)
		lineR.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		lineR.BorderSizePixel = 0
		lineR.Active = false
		lineR.ZIndex = 9806
		lineR.Visible = (mode == "line" or mode == "dline")
		lineR.Parent = row
		res.lineR = lineR
		...
		name.Font = s.Bold and Enum.Font.GothamBold or Enum.Font.Gotham
	...
	local function setRowStyle(res, color)
		local s = States.FeatureHUD
		res.bg.Visible = s.ShowBg ~= false
		res.bg.BackgroundTransparency = (s.ShowBg ~= false) and 0.22 or 1
		res.name.Font = s.Bold and Enum.Font.GothamBold or Enum.Font.Gotham
		res.name.TextColor3 = color
		res.lineL.BackgroundColor3 = color
		res.lineR.BackgroundColor3 = color
	end
	...
	local function removeRow(key)
		...
		pcall(function()
			...
			tween(row, {Position = UDim2.new(0, 50, 0, y)}, TweenSlide)
			if res and res.name then tween(res.name, {TextTransparency = 1}, TweenFast) end
			if res and res.bg then tween(res.bg, {BackgroundTransparency = 1}, TweenFast) end
			task.delay(0.3, function()
	...
	local function layout()
		...
		local s = States.FeatureHUD
		local list = sortedKeys()
		F.Order = list
		local n = #list
		local wid = 24
		for _, k in ipairs(list) do
			wid = math.max(wid, measurePx(nameOf[k] or k))
		end
		local frameW = wid + 18
		local totalH = DRAG_H + n * (ROW_H + GAP) + PAD - GAP
		if totalH < DRAG_H + ROW_H then totalH = DRAG_H + ROW_H end
		-- 顶部线条(线条模式)
		if s.Mode == "line" then
			local tl = makeTopLine()
			tl.Visible = true
			tl.Size = UDim2.new(0, frameW, 0, 3)
		else
			if F.TopLine then F.TopLine.Visible = false end
		end
		tween(F.Frame, {Size = UDim2.new(0, frameW, 0, totalH)}, TweenSmooth)
		for i, k in ipairs(list) do
			local y = DRAG_H + (i - 1) * (ROW_H + GAP)
			local row = F.Rows[k]
			if not row then
				local _, res = makeRow(k, frameW)
				row = F.Rows[k]
				row.Position = UDim2.new(0, 46, 0, y)
				res.name.TextTransparency = 1
				res.bg.BackgroundTransparency = 1
				tween(row, {Position = UDim2.new(0, 0, 0, y)}, TweenSlide)
				tween(res.name, {TextTransparency = 0}, TweenSmooth)
				if s.ShowBg ~= false then tween(res.bg, {BackgroundTransparency = 0.22}, TweenSmooth) end
			else
				local res = row.Resources
				if math.abs(row.Position.X.Offset - 0) > 1 or math.abs(row.Position.Y.Offset - y) > 1 then
	...
	local function sync()
		...
		local list = getEnabledOrder()
		local fixed = {}
		for _, k in ipairs(list) do fixed[k] = true end
		for key in pairs(F.Rows) do
			if not fixed[key] then removeRow(key) end
		end
		layout()
	end

	-- 动态渐变配色: 整体循环滚动, 每行按序号相位延迟形成向下流动的彩色水波
	local function applyColors(now)
		...
		for i, k in ipairs(list) do
			...
			local t
			if dir == "both" then
				local tri = math.abs(2 * (ph % 1) - 1)
				t = tri - (i - 1) * (RowDelay / CycleDur)
			elseif dir == "reverse" then
				t = ph + (i - 1) * (RowDelay / CycleDur)
			else
				t = ph - (i - 1) * (RowDelay / CycleDur)
			end
			local col = gradColorAt(t)
			local res = row.Resources
			res.name.TextColor3 = col
			res.lineR.BackgroundColor3 = col
			res.lineL.BackgroundColor3 = col
		end
		if F.TopLine then F.TopLine.BackgroundColor3 = gradColorAt(ph) end
	end
	...
	F.resetPos = function()
		...
		if F.Frame and F.Frame.Parent then
			local _, defPos = defaultPos()
			tween(F.Frame, {Position = defPos}, TweenSmooth)
			makeDraggable(F.Frame, F.Grip, function()
	...
	Updaters.FeatureHUD = function()
		if States.FeatureHUD.Enabled then
			...
			if F.Grip then F.Grip.Visible = true end
			sync()
			F.LastSig = sigOf(getEnabledOrder())
			...
			Conns.FeatureHUD = RunService.Heartbeat:Connect(function()
				...
				applyColors(os.clock() - baseTime)
			end)
		else
			unbind("FeatureHUD")
			if F.Frame then
				F.Frame.Visible = false
				if F.Grip then F.Grip.Visible = false end
